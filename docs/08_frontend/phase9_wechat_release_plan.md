...
| 9.5 公开发布安全与身份 | 关闭公众身份、安全和生产运维缺口 | 微信登录、账号关联规则、限流、refresh 轮换、Secret 管理、监控告警、生产存储、隐私与用户权利方案 | **仓库实现完成 / 外部证据待办**：身份与数据处理自动化已 Review；真实微信、集中 Secret、监控、对象存储及隐私平台材料仍阻断 Gate B |
...
2026-09-02 同时完成 9.5 不依赖备案/外部资源的仓库范围：服务端微信身份适配与
绑定规则、Refresh family 轮换/重放撤销、Redis 限流、账号注销匿名化、外部标识
HMAC、结构化安全事件、Secret 文件注入边界、图片存储 Protocol 和 MySQL 迁移均已
实现并通过自动化。真实 AppID 真机、集中 Secret Manager、告警送达、对象存储和
微信隐私材料没有伪造执行，详见 [Phase 9.5 基线](../09_release/phase95_public_security_baseline.md)。

本地最终证据为后端 `1693 passed, 9 skipped`、隔离 MySQL 8.0.46 Aerich 0→3
与 9 项门槛、前端 61 套件/392 项及全部静态/契约检查；微信登录模式非发布构建为
97 文件/605,381 bytes。该证据仍需由当前 SHA 的远端干净 CI 替代后才能进入 RC。
