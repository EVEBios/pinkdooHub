...
> 状态：**工程实现与微信开发者工具 Functional 已全部完成。** 本阶段只建立管理商品列表与详情的安全读模型，不包含任何 Product/Option/Image/Inventory mutation。
...
输入框和按钮编辑的是本地草稿；只有提交后才替换当前 Query。这样用户可以一次组合多个条件，也能明确理解何时发请求。筛选变化必须重置第 1 页，加载更多必须复用同一组已提交条件。
