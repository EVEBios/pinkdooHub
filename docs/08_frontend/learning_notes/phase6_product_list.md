...
- `[LOCAL-FE] 拼豆材料包 01` 会通过正式 `InventoryService.adjust_stock()` 建立初始库存 8，另外 5 条 Kit 保持 0，分别覆盖“允许加入”和“无库存禁用”。
...
重复运行时，完整的同名 Online 数据会被跳过；脚本不会重复制造 13 条。正库存 Kit 会原样保留，不会每次执行都再加 8。初始库存调整使用固定幂等键，并产生正式 Inventory 流水与 Audit；如果这笔初始库存以后被订单消耗为 0，旧幂等结果不能代表当前库存，脚本会明确停止并要求通过 Inventory 调整 API 补货。如果保留名称已被 Draft、Offline、逻辑删除或错误类型占用，命令也会停止，要求开发者先检查异常数据，而不是猜测如何覆盖。
...
库存尤其不能直接改 `product_kits.stock`。库存余额是当前状态，InventoryTransaction 是“为什么变成这个状态”的不可变历史，Audit 则记录“谁执行了管理动作”。只改余额会破坏三者的一致性，因此 Seed 也必须走正式 Inventory 用例。另一个容易混淆的知识点是：幂等重放返回的是第一次操作的结果，不保证当前资源仍处于当时状态；脚本在调整后重新读取 Product，才以当前库存作为最终判断。
