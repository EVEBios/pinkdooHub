...
| 10 | Reservation N1（实施/验收状态见 changelog） | 服务端日历、时区、状态/原因联合、隐私、批量命令与结果收敛 |
...
> **本版范围已冻结为微信小程序。** 第一交付目标是受控的内部微信测试版；通过后再以独立 Go/No-Go 门决定是否进入对外公开发布。支付宝、抖音和 H5 不属于本版 CI 阻断项、Functional 矩阵或发布承诺，已有跨端源码与构建能力只作为未来兼容基础保留。Phase 9.1–9.3 已完成；Phase 9.4 仍受备案、真实域名/证书、体验版权限与 iOS/Android 真机证据等外部条件阻断；Phase 9.5 中不依赖这些外部条件的仓库实现已完成，但 Gate B 外部资源与验收仍未完成。完整范围见 [Phase 9 微信小程序发布规划](phase9_wechat_release_plan.md)，可执行审计与清单见 [Phase 9 发布文档](../09_release/README.md)。
...
| 9.2 | 建立后端 SQLite、MySQL-only、前端质量、OpenAPI、微信构建、生成物/Secret/依赖审计 CI | **已完成（2026-08-31）**：历史远端候选 8/8 Job 通过；后续模块仍须在当前 SHA 重跑 |
| 9.3 | 在可销毁的生产相似环境完成 0→当前、受支持升级、备份恢复、失败处置和完整 Smoke | **已完成（2026-08-31）**：迁移、恢复、FastAPI、Redis、图片、管理员初始化和健康检查已有证据 |
| 9.4 | 上传受控微信体验版并在 iOS/Android 完成弱网、前后台、上传和角色 Functional | **外部阻断/未完成**：备案、真实域名/证书、体验版权限及真机矩阵未齐，不得上传或误报 Gate A Ready |
| 9.5 | 完成微信身份、账号关联、认证强化、Secret、存储、监控和隐私基线 | **仓库内可实施范围已完成（2026-09-02）**；真实 AppID、集中 Secret/存储/告警与合规材料仍是 Gate B 外部阻断 |
...
当前不是“进入 9.2”：9.1–9.3 已完成，9.4 因外部条件保持未完成，9.5 只完成仓库内可实施部分。持久 Gate A 已受控迁移到 M7，但 M8、共享/预发布/生产数据库和真实微信支付均未因此完成；微信后台仍未修改，也未上传或发布任何版本。Gate A 继续保持 No-Go，直到 9.4 外部条件及当前候选 SHA/M8 的完整回归与目标环境证据通过。

阶段完成标准以 [Phase 9 微信小程序发布规划](phase9_wechat_release_plan.md) 的 Gate A/Gate B 和[测试策略](testing_strategy.md)为准。

---

## 13. 阶段 10：Reservation N1

> **阶段状态：N1/M7 后端契约、仓库实现与 MySQL 门槛已完成，M5/M7 已进入当前 Gate A M7；前端六页面、API/Feature、入口与工程自动化已落地，真实客户端 Functional 待验收。** N1 只做“我的预约/详情”状态与 `customer_message` 可见、管理员读取当前手机号人工联系；不主动发送微信通知，微信订阅消息 N2 保持 Deferred。

学习：

- `Asia/Shanghai` 营业日期与 UTC 权威 datetime 的双表示；
- 服务端计算选项与客户端交互状态的区别；
- 四状态、两个原因字段和三个状态时间组成的可辨识联合；
- 无客户端幂等键写请求的 unknown 收敛，与 PUT 首次 201/重放 200 的差异；
- 列表手机号掩码、详情按需完整手机号和 Storage/日志隐私；
- 一次服务端日历请求避免逐日 N+1；批量店休只是一个命令，不在前端循环取消预约。

建议纵向切片：

| 子阶段 | 实践 | 完成标准 |
|--------|------|----------|
| 10.1 | Reservation OpenAPI 类型、Endpoint 与 Runtime Guard | 四状态/原因/快照/UTC-上海字段/Page/店休计数不变量通过；不手改生成类型 |
| 10.2 | booking-options 与创建 | 只发送三个字段；空日期、42251–42254、一次点击合并、明确重复意图生成不同 ID、unknown 和手机号补充引导通过 |
| 10.3 | 我的预约、详情、取消与改期引导 | Owner-only、服务端快照/customer_message、3 小时截止、empty-body、40951/40952 收敛通过 |
| 10.4 | ADMIN 列表/详情/审核 | 组合筛选、掩码/完整手机号隔离、fixed no_capacity、40951/40953 与权限通过 |
| 10.5 | 店休管理 | PUT 201/200 replay、取消计数、DELETE 40452、恢复不复活历史和人工联系提示通过 |
| 10.6 | 微信 Functional 与 Review | 周一、0–30 日、营业时间、价格快照、四状态、弱网/竞态/隐私矩阵通过；无 N2 假 UI |

完整 HTTP 形状见 [前端 API 集成契约](api_integration_contract.md) 与 [Reservation API](../03_api/reservation_api.md)。前端不得实现本地容量算法、法定节假日日历、订单/支付绑定或订阅通知。
