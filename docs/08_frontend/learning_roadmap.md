...
当前不是“进入 9.2”：9.1–9.3 已完成，9.4 因外部条件保持未完成，9.5 只完成仓库内可实施部分。尚未实现真实微信支付、未迁移持久数据库、未修改微信后台，也未上传或发布任何版本；Gate A 继续保持 No-Go，直到 9.4 及当前候选 SHA 的完整回归证据通过。
...
> **阶段状态：后端契约已冻结并形成仓库/M5 候选；前端六页面、API/Feature、入口与工程自动化已落地，真实环境 Functional 待验收。** N1 只做“我的预约/详情”状态与 `customer_message` 可见、管理员读取当前手机号人工联系；不主动发送微信通知，微信订阅消息 N2 保持 Deferred。
