...
| 顾客一级导航 | 商城 / 预约 / 购物车 / 会员中心四个固定根 Tab；微信自定义瓷白丝带托盘，其他端原生降级 |
...
7. 新功能至少通过类型、单元/组件、契约和当前发布平台构建门槛；Phase 9 本版为微信 `weapp`，其他平台恢复发布前重新建立各自门槛。
...
- `app.config.ts` 注册商城、预约、购物车、会员中心四个标准 Tab；微信设置 `custom: true`，由 `custom-tab-bar/` 渲染瓷白丝带托盘，支付宝、抖音和 H5 设置 `custom: false` 并使用原生 TabBar 兼容降级。
...
- 预约根页与订单二级页在普通 USER 再次显示时保留筛选并刷新第一页；会员中心重读资料和普通 USER 钱包；统一通过 `platform/use_refresh_after_page_return.ts` 只在真实 hide→show 后触发，避免首次挂载与首次显示重复请求。商城保留搜索、类型筛选和浏览上下文，不因 Tab 切换强制重置。
- 底栏只属于四个根页；商品详情、订单列表、下单确认、订单/预约详情、登录注册、钱包与全部管理页面保持普通页面栈且无底栏。该改造只改变前端路由/挂载边界，不改变后端 API、OpenAPI、数据库、业务权限或依赖。
...
- 顾客四个根页及商品详情、订单列表、下单确认、订单／预约详情和钱包流程通过 `styles/_customer.scss` 复用同一纹理身份区，并用受圆角裁切的内嵌强调带组织瓷白内容块。纹理不进入商品图片、业务状态、错误或流水正文；商城真实商品图仍是内容主体。
...
- 四项固定为商城、预约、购物车、会员中心，购物车为第三项，订单列表由会员中心进入；ADMIN+ 不消费这套顾客 Tab，而由默认启动分流和角色落点进入无底栏店铺工作台。本阶段不增加第五项或管理员底栏。
...
具体矩阵见 [测试策略](testing_strategy.md)。Phase 9 本版只发布微信小程序，完整阶段与 Gate 见 [Phase 9 微信小程序发布规划](phase9_wechat_release_plan.md)。架构级门槛：

1. TypeScript、Lint、Format、单元和组件测试通过；
2. OpenAPI 类型重新生成无未提交漂移；
3. 微信 `weapp` 生产构建、配置、Secret/Origin 扫描和包体检查通过；
4. 微信开发者工具与 iOS/Android 真机完成核心 Functional；
5. 支付宝、抖音和 H5 本版延后，不进入阻断项或发布承诺；未来启动对应平台时重新完成 Build + Functional + 真机；
...
剩余发布门槛（不属于 Spike，属于正式工程与发布阶段）：微信 CI 与可重复构建、生产相似 MySQL/Redis/备份恢复演练、内部体验版真机与弱网、合法域名/HTTPS、Secret/监控/隐私、公开版本的微信身份，以及在线收款时的微信支付闭环。支付宝、抖音和 H5 本版延后。

### 会员中心视觉精修（2026-09-13）

会员页将身份区与三状态订单入口放入同一个纹理页头；余额卡在其下，个人资料最后且默认完整展开。资料昵称／账号与账户字段分组，加入时间展示 UTC 日期，不修改原始契约值。钱包图案为透明 PNG，订单使用白色 Heroicons 轮廓图标；退出按钮使用浅粉胶囊，正常态不输出 disabled 属性，避免 H5 把 `disabled="false"` 匹配为禁用样式。既有角色守卫、钱包刷新／错误态、资金入口与根 Tab 行为保留。

余额卡二次修正：钱包图片使用正方形尺寸和 `aspectFit`，禁止非等比拉伸；右侧图案与金额居中排布，金额不换行。未开通充值改为静态状态文本，仅服务端 topup_enabled 且账户／钱包可用时呈现充值按钮；资金明细标题区仍保留独立触控目标。
