...
| 1013 | 当前账号未绑定该外部身份 |
| 1014 | 解绑会移除唯一登录方式 |
| 1015 | 存在 Pending/Paid 订单、未来活跃预约、处理中资金事实、可退款钱包结算敞口或非零钱包余额，暂不可注销 |
...
| 注销账号 | 保留业务外键并匿名化，不物理删除订单/预约/资金/库存/审计；Pending/Paid 订单、未来活跃预约、处理中资金事实、可退款钱包结算敞口或非零钱包余额时拒绝 |
...
`DELETE /api/v1/users/me` 仅允许普通用户。请求必须包含 `confirmation: "DELETE"`，并且在 `password` 与 `wechat_code` 中恰好选择一种二次验证方式。存在 Pending/Paid 订单、未来 `pending/confirmed` 且 `scheduled_end_at > now_utc` 的 Reservation、Pending Payment/RechargeOrder/Refund、尚可全额退款的钱包结算敞口或非 `0.00` 钱包余额时返回 1015。预约以结束时间而非开始时间判断，避免体验进行中匿名化当前手机号。可退款敞口包括未成功退款的 PAID 钱包结算，以及完成未满 30 天的 COMPLETED 钱包结算；因此余额为零也不代表可以注销。已完成/取消订单与 rejected/cancelled/已经结束的预约本身不阻塞，但其独立资金事实仍按上述条件检查。

成功返回 `{"code":0,"message":"Account deleted","data":null}`。数据库事务的 commit 是注销的权威成功点：事务内删除全部外部身份、递增 `auth_version`，将用户匿名化并改为 deleted，同时把已有 WalletAccount 改为 `closed` 并写入 `DELETE_ACCOUNT` Audit；用户主键和历史钱包流水、Payment、Refund、订单、预约、库存及审计保留。Reservation 不保存手机号快照，因此匿名化后历史预约的管理详情会返回 `user_phone=null`。
