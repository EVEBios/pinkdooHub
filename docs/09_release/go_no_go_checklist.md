...
> **2026-09-16 当前状态：Gate A M15 迁移与运维收口完成；微信发布仍为 No-Go。** 本候选 CI 9/9、M10–M15 升级、管理员验收、故障恢复、验收后独立恢复及 finalized 均通过；详见[M15 完成报告](reports/gatea_m15_completion_2026-09-16.md)。以下 A–G 旧候选的状态与未勾选恢复步骤为历史，不应据此重做迁移或删除保护证据。域名/HTTPS RC、真机、Gate B 与正式微信发布须各自满足原有门槛。
...
> **Historical Status (2026-09-12):** No-Go / Not Authorized — live Gate A 是旧候选 A/M9，B 留有 `prepared` pending，C 的 9/9 未能通过真实 isolated stage 且已零残留清理；只允许新候选 D 在自身全新 9/9 后按受控 A→B→D/M9→M9 前滚链恢复，容量、真实 HTTPS RC 与真机仍阻断公开发布
...
2026-09-11 当前覆盖说明：旧候选 A
`d6c09482ee0f5583d79bd847e995746c9c6ee1a3` 已把 live 配置、数据库和 App/Nginx/MySQL/
Redis/`table-sweeper` 带到 M9，但 `current` 仍是 finalized lineage S
`73dca350505d43775fb1ff1158ccf6aabc221998`。A acceptance 在创建订单后读取 T01 身份时
...
以下 M8/M7→M9 项是旧现场路径的历史清单，保留用于审计，但对当前 live A/M9 不得执行、
不得补勾，也不构成 B 接管步骤。当前恢复项接在该历史块之后。
