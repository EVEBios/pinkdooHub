...
> **2026-09-15 恢复更新：** D 的真实 stage 已因归档 `0755`／加载器 `0644` 契约不一致在 pending/Image/Release 写入前失败；下文 D 的待执行描述为历史状态。当前独立 M9 修复候选为 E，绑定解压后的精确模式并新增源码前置检查；E 自身远端 CI 与现场证据尚未取得，Gate A 保持 No-Go。历史交接来源、测试边界与执行顺序以 [M9 恢复记录](../09_release/reports/gatea_m9_recovery_preflight_2026-09-15.md) 为准，本文不把旧 Run 或旧现场状态视为本轮通过证据。
...
- M8 **数字色块 HEX 的 Model/迁移/API/销售校验、小程序直绘和发布工具均已完成仓库实现，并已应用当前本地持久 SQLite；Gate A live Schema 也已由旧候选 A 带入 M9，但尚未完成 M9 acceptance/finalize**：`BeadColor.swatch_hex` 为 nullable `VARCHAR(7)`，非空写入规范化为大写 `#RRGGBB`；code/name/有效 HEX 三者完整才算 configured，激活、商品颜色启用、上架、公共输出与下单均 fail closed。公开颜色响应要求非空 HEX，管理响应保留 nullable 诊断态。小程序以共享组件用 `backgroundColor` 直绘，并只在迁移期 HEX 缺失时回退可选 `swatch_image_url`；该 URL 只用于既有兼容 PNG 或未来实拍校色 WebP，不回退商品封面。现有 221 张 PNG 不转 WebP、不删除；纯数字色块不再生成新图片格式。
- M9 **二维码开台已于 2026-09-10 完成仓库实现**：精确建立 `T01`–`T30` 与环境专属随机 QR Token；NORMAL USER 将本人含 Experience 的 Pending 订单绑定桌台后占用 15 分钟。付款以 `Payment.succeeded_at` 为统一起点，Experience 按 `option_duration_minutes` 精确分组，同分钟合并、不同分钟分别计时，每组增加 10 分钟；`quantity` 不乘时长，Kit 可同单购买但不参与计时，纯 Kit 不可开台。最长 Timer 到期只释放桌台；取消、完成、退款和管理员释放在原事务内关闭会话。四表、M9 迁移、API/Mapper、严格幂等与限流、sweeper/reconcile、顾客扫码/付款/计时页和 ADMIN 桌台页均已实现。备案前普通占位码使用 `PINKDOOHUB_TABLE:v1:<token>`，目标微信版本为 `develop`；微信官方小程序码和真实微信支付分别 Deferred。当前 live config/DB/runtime 已是旧候选 A/M9，但 `current` 仍是 M7 lineage S，A acceptance 失败且尚未 finalize，不能视为已发布或已验收。
- M9 候选 D 当前本地门槛为 candidate 专项 `207 passed`、Release 完整 `1013 passed`、后端完整 `3135 passed, 39 skipped`；前端最近完整结果仍为 `102 suites / 719 tests`，TypeScript/ESLint/Stylelint/CI policy/OpenAPI 类型通过；一次性 MySQL 8.0.46 的 M0→M9 迁移与 M9 真实门槛为 `39 passed`。Runtime 关闭未经脱敏的 Uvicorn access log，Nginx 与应用异常日志对桌台 Token 路径统一脱敏；table reconcile 同时校验打开与已关闭会话的历史付款/计时快照。微信 CI 通过独立 `.env.ci` 隔离生产占位 Origin，页头纹理只在全局样式中内联一次，当前不可发布产物主包约 0.86 MiB、总包约 1.42 MiB。候选 C head `e909c42cebaf59931536ddc2c82a43f19a29925c` / merge target `c709d6252a07d65eb1457b23e7036ecb736f18b8` 的 Run 34616037853 已远端 9/9；候选 D 尚未取得自身远端 CI 或 stage 证据。
- Gate A Backup 的常驻服务恢复按精确 Schema 分流：M7/M8 不启动 M9 `table-sweeper`；M9 在备份前必须要求 sweeper 健康，并与 App/Nginx 一起停写和恢复。该分流只由备份器从批准的 Aerich 链内部选择，未知链或停写窗口内版本变化均拒绝；普通 `app-up` 仍默认严格要求 M9 sweeper。当前工具新建的 M8/M9 Backup 还必须以 `m8-swatch-content-v1` 按 `bead_colors.id` 顺序绑定 `id/slot_no/swatch_hex`，独立 Restore、异机副本、M9 resilience 与 finalize 都重算匹配；M9 另保留 `m7-preserved-business-v1` 和 `m9-table-business-v1`。早于该 profile 且 Backup/Restore/match 三个 swatch 字段全部缺失的历史 M8/M9 pair 仍可兼容读取与异机 export/verify，任何部分出现或不匹配都拒绝；all-absent 历史 pair 不能充当本次 A→D source、post-acceptance、resilience 或 finalize 新证据。
- M9 持久发布候选采用 `stage` → `activate-config` → upgrade/replay → `app-up` → acceptance → resilience → 数据后 Backup/Restore → `finalize` 的可恢复状态机。当前 A/M9+B/prepared 现场只允许全新 D 执行 schema v3 `stage(D, A failure digest, B stage/pending digests)`，再走八阶段 takeover retirement、新 A/M9 Backup/Restore、A→D activation、M9→M9 adoption/replay 与 D 验收链。退休阶段固定为 `prepared` → `write-free-verified` → `superseded-retirement-archived` → `acceptance-archived` → `record-published` → `superseded-retirement-removed` → `canonical-removed` → `runtime-restored`；无论 work/control error 都强制恢复并复验 A/M9 五服务，恢复/清理失败优先于原错误，延迟信号最后传播。A acceptance 与 B retirement archive 必须分别保留在两个 guarded 目录，成为 `activate-config`、`rollback-config`、adoption 与 `finalize` 持续重验的 lineage 依赖。adoption 必须零迁移、零 bootstrap、零数据库修复，并记录 `database_changes_applied=false`、`migrations_applied=[]`。唯一 operation lock 防并发，另以全局 inventory 扫描所有候选 SHA 的五类 candidate pending，以及所有候选的 acceptance pending/complete；candidate、Backup/Restore、upgrade、Bootstrap、两类代表数据、`initial-migrate`、`app-up`、acceptance 与 resilience 都必须在数据库快照、TTY、业务 API 或资源启停前 fail closed。只有原动作严格绑定 candidate/kind/path/checkpoint 的 own recovery 可获结构化 allowance，任一第二 blocker、畸形项或非普通路径仍失败；不同 Backup ID 不因此全局互斥。
- 退休 verifier 使用 D 镜像，输入只走有界、拒绝重复字段的 stdin JSON，真实 argv 必须为空且 schema version 必须是精确整数；它要求 fixture Kit 的全部库存流水恰为 SUPER_ADMIN supply `0→10`、Order deduction `10→9`、cancellation restore `9→10` 三条，并确认该 Order 没有 Payment/Settlement/Refund/WalletTransaction/TableSession。`app-up` 的 transition 与 acceptance 两个 own-recovery allowance 还必须交叉绑定同一 runtime candidate；恢复阶段的延迟信号不能遮蔽普通原始 work error。
...
- Run 34523689519 已为 head `41ad3cd...` / merge target `ceb664e...` 完成 9/9；持久 Gate A 随后成功 stage 并完成新 M7 Backup/Restore `20260911t013550z`，但按旧 Runbook 从已安装 Release 执行宿主 `python3 -m scripts.release.gatea_*` 会生成 `__pycache__`/`.pyc`，把冻结 manifest 从 968 改为 971 文件，`activate-config` 因此在零写入 plan 阶段 fail closed。现场当时仍为 M7 配置、M0–M7 数据库和四项健康服务，无 activation journal、无 M8/M9 迁移；该 target 不得通过手工删 cache 继续。随后修复把已安装 Release CLI 收口为 `python3 -B -m`，校验过的单文件 stage launcher 收口为 `python3 -I -B <launcher>`，保持全树 manifest 不忽略可执行 bytecode。该条冻结当时检查点；其后的 A/M9、B/prepared 与 C/pre-install 失败要求以本节后续条目为当前事实。
- 当前失败检查点：旧候选 A `d6c09482...` 已把 live config/DB/runtime 推进到 M9，`current` 仍为 S `73dca350...`。A acceptance 在 `order_created` 后读取桌台 bootstrap identity 时使用了未经过 Entrypoint Secret 加载的 Compose `exec` 进程，因而安全失败；补偿已取消 Order、下架 fixture、恢复 Kit 库存并撤销会话，没有资金或桌台 Session/Timer/Occupancy，schema v3 pending 保留。读取器现改为保留默认 Entrypoint 的 `run --rm --no-deps app`；B `ad2ac8...` 已 stage，但 retirement 只到 `prepared`。候选 C 虽由 Run 34616037853 取得 9/9，真实 schema v3 stage 却在 pre-install predecessor validation 误调用 `_runtime_modules()`，隔离 launcher 报出 `activation must run from an installed candidate release`；该失败发生在任何 C pending/Image/Release 写入前。清理后 S/A/B、三份受保护 digest 与五服务均不变，C 无现场残留。
- 候选 D 的 stage 修复只在 source archive、launcher、CI artifact 与完整 GitHub provenance 已验证后，先执行第二次 blocker scan，再以 stable no-follow exact-bytes 加载归档内标准库限定的 operations validator；不允许通过 `PYTHONPATH`、`current`、A/B Release 或现场工作树 fallback，未通过 predecessor validation 前不得写 pending 或 build Image。D 必须先取得自己的全新 9/9；禁止复用 C 的 Run 或重新使用 C target。
- 发布证据采用 durable no-clobber 语义：不可变 Record/artifact 先在同目录随机临时文件完整写入并 file `fsync`，再 hard-link 发布并同步目录；candidate/config/current journal、Backup/Restore、Upgrade evidence/success、acceptance `.complete`/final 和 resilience final 都按各自提交点恢复。acceptance 使用 schema v3 pending 与独立的 schema v1 success，Payment ID/No 哈希/成功时间跨两者绑定；resilience final 已可见时只做两次完整现场一致性复验并清理最多一个同 inode writer temp，孤儿/额外 alias/sidecar 阻断且不会重跑依赖故障。所有持久变更入口在 CLI work 前统一安装 SIGHUP/SIGTERM/SIGINT guard，SIGINT 保持 `KeyboardInterrupt`；强制恢复/清理子进程使用新 session/独立进程组。服务 stop、Restore project 与 candidate 临时镜像分别以 Compose `ps`、容器/两个临时卷/internal network inventory、精确 image reference inventory 判定完成，后两类最多重试两轮；不能以 Docker 命令退出码代替复核。错误传播固定为 recovery/cleanup failure > original work/control error > deferred signal。SIGKILL/断电后只能严格恢复 journal/已提交 Record，不能猜测完成。M9 resilience 必须显式传入 acceptance Record 与确认 SHA-256，并在前后直连验证 candidate/Image、Operations/CI、upgrade/replay、代表数据/凭据、五服务、attempt digest 与完成时间；M9 Record 为 schema v2，M2/M7/M8 legacy Record 保持 schema v1。当前 live M9 仍是 No-Go，直到 D 完成 acceptance、resilience、数据后 Restore 与 finalization。
- schema v2 adoption 的崩溃恢复只有一个例外：evidence 已完整耐久为 `succeeded/completed`、canonical success 尚未发布时，可带原 A/S/D、Backup、manifest、retirement 与全部 apply 确认值，重验服务停写、lineage/Record、数据库/图片/M7/M8/M9 内容和 reconcile 后 no-clobber 补发 success；该路径不重跑迁移或业务写。失败/未完成 evidence、身份或现场漂移都保持 No-Go。`app-up` 同时对同一 candidate 的 initial-migration 与 existing-database upgrade Record 执行严格 XOR；双份、零份或 symlink 在任何 Compose 启停前阻断，不选择“较新”或“看似匹配”的一份。
- MySQL/Aerich 权威链现为 M0–M9；M8 负责 221 色 HEX 回填与 MARD 精确核验，M9 新增固定桌台、会话、计时器和当前占用四表，并在迁移后执行 30 桌 bootstrap/replay。M8 的一次性 MySQL、历史 CI 与 M7→M8 updater 证据继续作为基线；live Gate A 已由 A 应用 M8/M9 并运行五服务，但 A acceptance 在开台前失败，`current` 仍指向 S。D 必须用自身干净 SHA 的 CI 和 M9→M9 adoption/现场 Record 证明，不得再以 M7 为数据库起点，也不得复用 C 的 9/9。
...
- Reservation N1/M7 **契约、仓库实现、MySQL 门槛和 Gate A 持久验收已完成**：独立预约不创建 Order/Payment；正常普通 USER 以 `experience_option_id + reservation_date + start_time` 创建 pending，必须有当前手机号。服务端按上海时区执行未来 0–30 日、11:00–20:00、至少提前 3 小时和可配置固定店休。单日/固定店休原子取消尚未开始的活跃预约为 `store_closed`，恢复不复活历史。M5/M7 已在 Gate A 的 M7 阶段应用并有综合数据/恢复证据，当前 live M9 继续继承；共享、预发布、生产及真机不因该结果自动通过。N2 微信订阅消息、Outbox、Worker 与主动通知保持 Deferred。
...
- 当前仍为 **No-Go**：Gate A 最后已 finalized 的 lineage 是 M7 Runtime S `73dca350...`，但 live config、数据库和五服务已是旧候选 A/M9；A failure pending 与 B `prepared` pending 原样保留。C 已取得自身 9/9，但真实 stage 在零现场写入前暴露隔离加载错误并清理完毕，所以不能继续。唯一后续是全新 D 自身 CI 9/9 后，以 schema v3 绑定 A/B 两份 pending 和三份 digest，完成八阶段 takeover、新 A/M9 Backup/Restore、A→D 零迁移 adoption、D acceptance/resilience、post-acceptance Backup/Restore，并把 `current` 从 S finalize 到 D。禁止手工删除 pending、临时注入 Secret、重跑 M7→M9、downgrade 或拼接 B/C/旧 Run 证据。真实 Origin、`release_eligible=true` RC、iOS/Android 真机、微信官方小程序码、upload/gray/release 和真实微信支付仍未完成或未授权；共享、预发布和生产环境未触碰。
- Phase 9.5 **不依赖备案的仓库实现已于 2026-09-02 完成**：后端已实现服务端微信 code2Session、微信首次普通用户创建、既有账号显式绑定/冲突/解绑、外部标识独立 Pepper HMAC、Refresh family 原子轮换与重放撤销、认证限流 fail-closed、账号注销匿名化和脱敏安全事件；密码/微信既有身份登录及微信首次注册唯一冲突收敛均锁定并复验 User，`last_login_at` 与登录 Audit 同事务，签发后再锁定确认 `auth_version/status`，不一致时只撤销本次新 family。注销以数据库 commit 为权威成功点，commit 后 Redis family 清理为 best-effort；失败仍返回成功，`status/auth_version` 保持会话失效，并记录不含 Token/JTI 的高优先级安全事件供运维重试清理。Order 创建也先锁后复验 NORMAL 普通 USER，封闭注销竞态。小程序新增显式 password/wechat 模式和 refresh 双 Token 替换。MySQL 迁移 3 已离线生成、在可销毁 MySQL 验证，并已随 Gate A 的 M3→M7 阶段升级应用、由当前 live M9 继承；共享、预发布和生产环境不因该证据自动迁移。`ImageStorage` 与 Secret 文件注入只完成供应商无关边界；真实 AppID 真机、集中 Secret Manager、告警送达、对象存储和微信隐私材料仍是 Gate B 阻断，未连接微信后台或创建云资源。权威边界见 `docs/09_release/phase95_public_security_baseline.md`。
...
- 当前代码版本候选仍为 **v0.6.0（尚未发布）**；Phase 4.1/4.2/4.3、Wallet/Payment/Refund v1、Reservation N1、M6/M7/M8 与 M9 二维码开台均已完成仓库实现。旧候选 A 已在持久 Gate A 实际执行 M7→M8→M9，使 live config、数据库和五服务达到 A/M9；但 A acceptance 失败，`current` 与最后已 finalized lineage 仍是 S/M7。B 停在 `prepared`，C 的隔离 stage 已零写入失败并清理；当前等待的是 D 自身 CI、A→B→D takeover、M9→M9 零迁移接管、D acceptance/resilience、数据后恢复与 finalization，而不是再次执行 M7→M9。
...
- MySQL 8+ M0–M9 迁移链已离线生成；M9 新增四张桌台表、四维当前占用 UNIQUE、RESTRICT 历史外键及 30 桌受控 bootstrap。M8 历史 MySQL/CI/updater 证据继续有效，但不能替代 D 当前候选的 CI 或其他持久环境迁移。Gate A live 数据库已由 A 应用 M8/M9 并精确达到 M0–M9，只是尚未通过 acceptance/finalization；共享、预发布和生产数据库仍均未因此自动应用。
...
| `product_kits.kit_kind` VARCHAR(20)（本地 SQLite 与 Gate A M7 阶段已应用，当前 live M9 继承） | `"fixed"` / `"color_selectable"` | `KitKind(str, Enum)` |
...
| `inventory_transactions.transaction_type` VARCHAR(40)（M4 增加退款恢复；Gate A M7 阶段已应用，当前 live M9 继承） | `"opening_balance"` / `"admin_adjustment"` / `"order_deduction"` / `"order_cancellation_restore"` / `"order_refund_restore"` | `InventoryTransactionType(str, Enum)` |
| `inventory_transactions.source_type` VARCHAR(30)（Gate A M7 阶段已应用，当前 live M9 继承） | `"migration"` / `"admin"` / `"order"` | `InventorySourceType(str, Enum)` |
| `wallet_accounts.status` VARCHAR(32)（Gate A M7 阶段已应用，当前 live M9 继承） | `"active"` / `"closed"` | `WalletStatus(str, Enum)` |
| `wallet_transactions.transaction_type` VARCHAR(32)（Gate A M7 阶段已应用，当前 live M9 继承） | `"recharge"` / `"order_payment"` / `"admin_adjustment"` / `"refund"` | `WalletTransactionType(str, Enum)` |
| `payments.purpose` VARCHAR(32)（Gate A M7 阶段已应用，当前 live M9 继承） | `"order"` / `"recharge"` | `PaymentPurpose(str, Enum)` |
| `payments.method` VARCHAR(32)（Gate A M7 阶段已应用，当前 live M9 继承） | `"wallet"` / `"wechat"` / `"manual"` | `PaymentMethod(str, Enum)` |
| `payments.status` VARCHAR(32)（Gate A M7 阶段已应用，当前 live M9 继承） | `"pending"` / `"succeeded"` / `"failed"` / `"closed"` | `PaymentStatus(str, Enum)` |
| `refunds.status` VARCHAR(32)（Gate A M7 阶段已应用，当前 live M9 继承） | `"pending"` / `"succeeded"` / `"failed"` | `RefundStatus(str, Enum)` |
| `reservations.status` VARCHAR(32)（Gate A M7 阶段已应用，当前 live M9 继承） | `"pending"` / `"confirmed"` / `"rejected"` / `"cancelled"` | `ReservationStatus(str, Enum)` |
| `reservations.rejection_reason` VARCHAR(32)（Gate A M7 阶段已应用，当前 live M9 继承） | null / `"no_capacity"` | `ReservationRejectionReason(str, Enum)` |
| `reservations.cancellation_reason` VARCHAR(32)（Gate A M7 阶段已应用，当前 live M9 继承） | null / `"customer_request"` / `"store_closed"` | `ReservationCancellationReason(str, Enum)` |
...
| Wallet/Payment/Refund | 4044x / 4094x / 4224x | 40441-40444 / 40941-40948 / 42241（40947 退款预留容量、40948 超退款窗口；M4 已在 Gate A M7 阶段应用并由 live M9 继承） |
| Reservation | 4045x / 4095x / 4225x | 40451-40452 / 40951-40953 / 42251-42255（N1 已实现；M5 已在 Gate A M7 阶段应用并由 live M9 继承） |
...
- MySQL 8+ 真实并发验证是 v0.6.0 发布硬门槛；Phase 4.3.11 的隔离实例门槛已通过且该次验证本身未写持久库，Inventory M2 后续已在 Gate A M7 阶段应用并由当前 live M9 继承；版本发布及其他持久环境迁移仍未自动发生。
...
- `migrations/models/2_20260814104655_add_inventory_transactions.py` 使用 `AERICH_MYSQL_VERSION=8.0` 离线生成，人工移除 `IF NOT EXISTS` 并声明 `RUN_IN_TRANSACTION=False`；已在一次性 MySQL 8.0.46 完成真实升级/降级/带数据再升级，未 fake，并已作为 M2 在 Gate A M7 阶段应用、由当前 live M9 继承。其他持久环境仍须各自受控执行。
...
- `app/models/order.py` 已实现 `Order` / `OrderItem`、`SmallIntField` 状态、订单号唯一约束、Decimal 快照、四条 `RESTRICT` 历史外键和五组稳定查询索引；MySQL 8+ 增量迁移已离线生成并静态 Review，且作为 M1 已在 Gate A M7 阶段应用、由当前 live M9 继承，其他持久环境须分别核验。
