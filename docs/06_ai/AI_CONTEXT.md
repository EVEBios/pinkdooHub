...
- M6 细节契约保持不变：每色数量≤99、颜色行≤20、非颜色行≤10、总行≤30；Product 上架与颜色启停共用 Product 行锁，全局色板修改按引用 Product ID 升序→BeadColor→启用关联锁序并锁后复验；管理输出保留 enabled + inactive/unconfigured 诊断态，公共输出 fail-closed。BeadColor.sort 范围为 `0..32767`，最长元数据审计使用有效 JSON 的 len/SHA-256 有界摘要；miniapp 与 OpenAPI 生成物已同步。
...
- 本地综合 demo seed 已实际应用并通过专用 verifier：写前备份 `backups/local-demo-data/db.sqlite3.pre-local-demo-20260907-105320-455438.bak` 与被 Git 忽略的 `backups/local-demo-data/synthetic-credentials.json` 均为 `0600`，不记录凭据值。新增 9 个合成用户；实际表为 users 13、products 19、product images 24、orders 8/items 10、payments 6/settlements 6/refunds 2、inventory transactions 238、reservations 7、wallet accounts 11/transactions 9、audit logs 583、external identities 0。订单覆盖四状态，预约 seed 覆盖四状态及 `customer_request/store_closed`，自选颜色 Kit 启用三色，每周固定店休为周三。`wallet_reconcile` 为 `scanned=11 mismatches=0 violations=0`；真实微信外部身份和 Provider 成功事实不伪造。
- 当前完整回归基线为后端 `2000 passed, 30 skipped`（113.05s）、Wallet/CI 契约专项 `164 passed`、`compileall` 通过；30 项 skip 为三类显式 MySQL-only 门槛，已由一次性 MySQL 联合 `30 passed` 覆盖。既有前端 Node 24.13.0 / npm 11.6.2 基线为 `83 suites / 562 tests`、TypeScript、ESLint、Stylelint 与 CI policy `17/17` 全绿。无新依赖或版本提升。
- 当前仍为 **No-Go**：Gate A 在 2026-09-02 的最后留证为非空 M2，当前真实状态尚未重新只读确认；现有 `initial-migrate` 仅支持空库，无已批准 M2→M7 升级入口；MARD 缺持久 MySQL/对象存储导入；Wallet-expanded workflow 尚未远端复现，M4/backfill/reconcile 未应用目标环境，真实 Origin/RC、iOS/Android 真机和微信外部条件均未完成。旧 M7 SHA 的远端 8/8 不能替代新资金门槛证据。
...
- 当前代码版本候选仍为 **v0.6.0（尚未发布）**；Phase 4.1/4.2/4.3、Wallet/Payment/Refund v1、Reservation N1、M6 自选颜色 Kit 与 M7 可配置每周固定店休均已完成仓库实现。M6/M7 一次性 MySQL 8.0.46 候选门槛和当前 head 远端 Run 34129910349 的 8/8 已通过，但 M4–M7 均未应用 Gate A、共享、预发布或生产 MySQL。
...
- MySQL 8+ M0–M7 迁移链已离线生成；本地提交 `58d8435` 对应候选已在一次性 MySQL 8.0.46 完成 Aerich 0→7、M0–M6 各历史起点→M7、M6/M7 snapshot 和联合 `21 passed`，实例已销毁。包含该修复的 head `4d6430c...` 已由远端 Run 34129910349 取得 8/8；任何迁移仍未因本轮工作应用到 Gate A、共享、预发布或生产数据库。
