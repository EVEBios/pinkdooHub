...
- 前端 **Phase 9.1–9.2 已于 2026-08-31 Complete；下一步为 9.3 隔离发布演练**：Draft PR #2 的 GitHub Actions Run 33355935212 绑定实现 head `23a0f08...`，8/8 Job 全部通过，覆盖 SQLite、MySQL 8.0.46 Aerich 0→1→2 与 9 项门槛、前端、OpenAPI、微信 artifact、双依赖审计和仓库卫生。真实 Runner 暴露并关闭 `.venv` 硬编码、Taro 根/产物配置平台差异、production npm 安装遗漏构建期 devDependencies 和 `tee` 吞错。仓库固定 Python 3.10.9、Node 24.13.0、npm 11.6.2/pip 26.2.1；Python 只剩 ecdsa 在固定 HS256 路径不可达的 2026-11-30 到期例外，npm 官方生产树 10 包/5 叶子公告也由同日到期精确策略 fail-closed。远端微信 artifact 为 97 文件/603,619 bytes，绑定 manifest/配置 SHA 且 `release_eligible=false`。真实 HTTPS/合法域名、备份恢复、readiness、图片持久化、SUPER_ADMIN bootstrap 和真机仍待 9.3–9.4 关闭；未授权持久迁移、微信后台变更、上传、提审或发布。
