...
- 当前仍为 **No-Go**：Gate A 权威成功点为 M7 Runtime `73dca350...` / Backup `20260908t021224z`；M8 基线虽已远端 8/8，仍未应用。`initial-migrate` 只支持空库；新 M7→M8 候选入口、21 表 M7 保留内容摘要、停写源精确预检和 Online no-op 复验已经实现并通过本地 Release 测试。独立只读代码审查曾发现成功重放没有重新证明 App/Nginx 仍停服；修复并补齐状态 fail-closed 矩阵后复核无未解决 P0–P3，但该结论只绑定本地 diff；仍须形成最终干净 SHA，并由该 SHA 的远端 CI 与完整 updater 隔离 MySQL 复现关闭自动化门槛，也未取得任何持久写授权。执行前仍须绑定带版本化内容摘要的新 Backup/Restore、目标 SHA/Image、停写和当次授权；成功后须在 `app-up` 前紧邻完成 live replay verification。保护依赖维护窗口内无直接 SQL/宿主图片旁路写入，不是跨 DB/文件系统绝对原子。禁止默认 M2 路径、直接内部原语、手工改表或临时改变商品状态。M8 HEX/gzip Runtime、数据后恢复、真实 Origin/RC、iOS/Android 真机和微信外部条件均未完成。
...
- 当前代码版本候选仍为 **v0.6.0（尚未发布）**；Phase 4.1/4.2/4.3、Wallet/Payment/Refund v1、Reservation N1、M6 自选颜色 Kit、M7 可配置每周固定店休与 M8 数字色块 HEX/gzip 均已完成仓库实现。M8 基线的一次性 MySQL 和 head `4e745848...` 的 Run 34242753255 已通过；其后的 M7→M8/Online no-op 发布保护仅有本地候选证据，尚待新远端 Run。Gate A 已到 M7并完成 Wallet/MARD，只有 M8 仍待持久应用。
...
- MySQL 8+ M0–M8 迁移链已离线生成；一次性 MySQL 8.0.46 已真实完成 Aerich 0→8、M8 精确 221 HEX 与 Gate A MARD publish/replay，实例已销毁且 13308 已释放。head `4e745848...` 的远端 Run 34242753255 覆盖当时 M8 基线并为 8/8；后续 M7→M8/Online no-op 保护尚待新 Run。M8 仍未应用 Gate A、共享、预发布或生产数据库。
