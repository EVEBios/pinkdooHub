...
- 当前仍为 **No-Go**：Gate A 在 2026-09-02 的最后留证为非空 M2，当前真实状态尚未重新只读确认；现有 `initial-migrate` 仅支持空库，无已批准 M2→M7 升级入口；MARD 缺持久 MySQL/对象存储导入；钱包扩展 MySQL 门槛、backfill/reconcile、当前 SHA 远端 8/8、真实 Origin/RC、iOS/Android 真机和微信外部条件均未完成。
...
- 当前代码版本候选仍为 **v0.6.0（尚未发布）**；Phase 4.1/4.2/4.3、Wallet/Payment/Refund v1、Reservation N1、M6 自选颜色 Kit 与 M7 可配置每周固定店休均已完成仓库实现。M6/M7 一次性 MySQL 8.0.46 候选门槛已通过，但 M4–M7 均未应用 Gate A、共享、预发布或生产 MySQL，远端当前 SHA 8/8 也未重跑。
...
- MySQL 8+ M0–M7 迁移链已离线生成；本地提交 `58d8435` 对应候选已在一次性 MySQL 8.0.46 完成 Aerich 0→7、M0–M6 各历史起点→M7、M6/M7 snapshot 和联合 `21 passed`，实例已销毁。该候选未 push/远端重跑，任何迁移也未因本轮工作应用到 Gate A、共享、预发布或生产数据库。
