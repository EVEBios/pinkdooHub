...
- Run 34523689519 已为 head `41ad3cd...` / merge target `ceb664e...` 完成 9/9；持久 Gate A 随后成功 stage 并完成新 M7 Backup/Restore `20260911t013550z`，但按旧 Runbook 从已安装 Release 执行宿主 `python3 -m scripts.release.gatea_*` 会生成 `__pycache__`/`.pyc`，把冻结 manifest 从 968 改为 971 文件，`activate-config` 因此在零写入 plan 阶段 fail closed。现场仍为 M7 配置、M0–M7 数据库和四项健康服务，无 activation journal、无 M8/M9 迁移；该 target 不得通过手工删 cache 继续。当前修复把所有宿主 Release CLI 和校验过的 stage launcher 显式收口为 `python3 -B`，保持全树 manifest 不忽略可执行 bytecode，并新增真实 stage + fresh interpreter 回归；仍需全新 SHA 的 9/9 和新 target stage。
