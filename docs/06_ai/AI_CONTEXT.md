...
- 当前完整回归基线为后端 `2000 passed, 23 skipped`（112.25s）、退款专项 `49 passed`、`compileall` 通过、`pip check` 无破损依赖；前端 Node 24.13.0 / npm 11.6.2 下 `83 suites / 562 tests`、TypeScript、ESLint、Stylelint 与 CI policy `17/17` 均通过。无新依赖或版本提升。
- 当前仍为 **No-Go**：Gate A 在 2026-09-02 的最后留证为非空 M2，当前真实状态尚未重新只读确认；现有 `initial-migrate` 仅支持空库，无已批准 M2→M7 升级入口；MARD 缺持久 MySQL/对象存储导入；钱包扩展 MySQL 门槛、backfill/reconcile、真实 Origin/RC、iOS/Android 真机和微信外部条件均未完成。当前 SHA 远端 8/8 已关闭。
