...
- 前端 **Phase 7.4 ADMIN 订单查询与人工 Paid/Completed 工程实现及微信 Functional 均已完成**（2026-08-24）：`OrderApi` 新增 ADMIN 列表/详情/paid/complete，7 个 Query 严格投影，管理响应只增加 `user_id/user_nickname`，两个 PATCH 无 body 且校验目标状态。`admin` 分包提供完整状态/订单号/用户/UTC 日期筛选与服务端分页；结束日转次日 UTC 排他上界。首页只为 ADMIN+ 显示入口，列表/详情在角色确认后才挂载 Hook；后端 ADMIN+ 仍是授权事实。详情只派生 Pending → Paid、Paid → Completed，终态无按钮；命令 Promise 合并，unknown 不重发，成功/40921 后 GET 权威详情且 GET 完成前保持 submitting，paid/complete 不改库存。完整前端 31 套件/213 项、typecheck、全 src ESLint/Stylelint、OpenAPI 漂移、Order API 107 项、完整后端 1445 项（9 项 MySQL-only 跳过）及四端 production build 全部通过。2026-08-25 用户确认人工清单全部通过，包括断网 unknown 不重发、独立 Swagger 客户端制造的 40921 后 GET 收敛、普通用户 ADMIN API 403/不 refresh；Slow 3G 约 310 ms 返回而未触发 timeout，严格 timeout 只保留为非阻断补测。H5 入口 359 KiB、主 JS 276 KiB，保留体积、React Test Utils act 与 `[hash]` 上游告警。没有后端、数据库、依赖或生成 Schema 变化。
- Phase 7.1–7.4 已收口。下一步冻结并规划 Phase 8 第一条 ADMIN Product 最小纵向切片。H5 等后端 CORS allowlist 后验证。Order create 仍无客户端幂等键；微信登录/微信支付未实现、refresh 不轮换及登录/注册不限流均是明确集成/发布缺口。
