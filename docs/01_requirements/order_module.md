...
普通待付款与有效待付款桌台按首页主入口去重。管理员 paid 订单进入履约事项，pending/succeeded Refund 排除；服务中的桌台订单仍待履约但明确标“服务中”。首次人工付款、余额付款及订单完成生成顾客结果，和订单/资金共同提交。付款附带的桌台计时或关闭合并为一个结果。代客下单、调账与退款结果留待 P1.3；提醒本身不增加订单自动取消、退款审批或支付能力。详见 [提醒需求 P1.2](attention_module.md)。


### 统一购物选桌（2026-09-14）

订单确认页可选本次桌台：扫码购物自动预选，商城购物可扫码或输入桌号，未选择仍为普通购买。选择桌台的新订单须在同一事务中创建订单和待支付会话；原订单详情也可为未绑定/已超时的待付款体验订单选桌。支付与计时、超时限制、账号隔离及未知提交恢复以 [桌台业务规则](table_session_module.md#统一购物结算与选桌2026-09-14) 为准。无需数据库结构变化；不支持已支付普通订单事后补开台。

### 2026-09-14 · P1 并发选台补充

创建关联桌台的订单时，在取得桌台锁后以当前读检查占用，不能继续使用等待锁之前的 MySQL 可重复读快照。两位顾客同时选择同一桌台时，只允许一个订单和占用提交；另一方得到桌台已占用的业务错误，不留下订单、扣库或内部唯一键错误。锁占用时不联表锁另一位顾客的订单，保持既定锁序。
