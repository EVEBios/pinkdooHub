...
2026-09-14 新增的 [首页待办、结果提醒与微信通知整体规划](home_attention_notification_plan.md) 覆盖站内跨模块交接并列出未来微信候选事件；其中预约站内闭环 P1.1 已完成仓库实现，详见 [提醒需求](attention_module.md)，其余阶段待执行。它不改变本文仅规划店休取消的 N2 范围，不表示确认/拒绝、资金消息或管理员微信通知已经获批或实现；后续扩展须按新规划同步评审本文的范围与订阅规则。
