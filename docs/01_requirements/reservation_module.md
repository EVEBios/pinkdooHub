...
| `rejected` | 已拒绝 | ADMIN+ 以 `no_capacity` 拒绝 `pending` | 无 |
...
| `rejected/no_capacity` | 很抱歉，您选择的时段当前已无空位，本次预约已被门店拒绝。您可以选择其他日期或时段重新预约。 |
...
## M12 补充：预约站内提醒 P1.1（2026-09-14）

预约确认、拒绝以及单日/固定店休取消，与本人结果事件在同一事务提交；顾客取消已确认预约生成门店共享变更。取消待确认预约仅自然消除审核待办。原预约状态、权限、时间边界、锁序及电话联系责任不变。

可审核与已过时数量依据服务端时间派生，站内已读独立于预约状态及电话联系。详情显示相同版本后才确认明确事件，门店共享取消变更须点“已知悉”。完整规则以 [提醒需求](attention_module.md) 为准；微信主动通知仍按 N2 延后。此补充仅为仓库实现，不表示目标环境已应用 M12。

### 过期展示与管理筛选补充（2026-09-14）

未在开始前受理的 pending 在前端显示“已过期未处理”；管理端“待确认”与“已过期”分别查询未开始和已开始的 pending，后端分页前过滤，不改变历史业务状态。详见 [提醒需求修订](attention_module.md)。
