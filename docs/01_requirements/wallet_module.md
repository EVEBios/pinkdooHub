...
## P1.3 站内资金结果（2026-09-14）

管理员代客下单、余额调整及全额退款在原资金事务中写入站内结果；幂等重放不另发，失败不留成功消息。代客下单和退款按订单合并，独立调账按原始流水逐笔提醒。线下退款只说明门店登记，不能声称系统验证到账。完整展示与已读规则以 [提醒需求 P1.3](attention_module.md) 为准，接口见 [提醒 API](../03_api/attention_api.md)。不改变原扣款/退款权限、限额、余额守恒、库存、审计或微信资金开关。
