...
| 注销账号 | 二次验证，且无待支付/已支付订单、处理中资金、可退款钱包敞口或非零钱包余额时匿名化并撤销全部会话 |
...
|------|----------|
| 游客 | 浏览商品、注册、登录 |
| 普通用户 | 游客权限 + 个人资料管理 + 创建/查看自己的订单 + 自己的钱包与余额支付 |
| 管理员 | 管理用户、商品、订单和普通客户钱包；不继承普通客户的钱包能力 |
...
- 注销只允许普通用户，必须提交固定确认词并使用当前密码或当前微信 code 二次验证；Pending/Paid 订单、处理中 Payment/RechargeOrder/Refund、尚可全额退款的钱包结算敞口或非零钱包余额存在时均拒绝。可退款敞口包括未成功退款的 PAID 钱包结算，以及完成未满 30 天的 COMPLETED 钱包结算；因此即使当前余额为 `0.00` 也不得关闭钱包。
- 注销保留 User 主键、钱包、资金流水、Payment、Refund、订单、库存流水和审计外键，但删除外部身份并匿名化 username/nickname/phone/avatar/password，状态改为 deleted；已有钱包同步改为 closed。该处理是为履约、财务与安全审计保留最小必要记录，不等于物理删除全部历史业务数据。
