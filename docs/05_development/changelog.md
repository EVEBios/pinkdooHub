...
- 当前 A→B→C 阻塞恢复候选的本地门槛为后端完整 `3130 passed, 39 skipped`、Release 完整 `1008 passed`；新增覆盖 verifier schema v2、schema v3 三重绑定 stage、双 pending typed allowance、双原始证据归档、八阶段崩溃恢复，以及 M9→M9 零迁移 adoption/replay。前端最近完整结果仍为 `102 suites / 719 tests`，TypeScript、ESLint、Stylelint、CI policy 和 OpenAPI 类型均通过；一次性 MySQL 8.0.46 从 M0→M9 后的 M9 迁移/约束/并发/领域门槛仍为 `39 passed`，完整迁移编排与资源清理通过。这些本地与旧远端证据均不能替代 C 自身的全新远端 9/9。微信 CI 改用独立 `.env.ci`，避免生产占位 Origin 覆盖 CI 注入值；页头纹理提升为全局 CSS 变量后只内联一次，当前不可发布微信产物主包约 0.86 MiB、总包约 1.42 MiB。Gate A 目标的非 Secret 标识保持 `ubuntu@118.195.195.59`，微信内部验收环境为 `develop`；私钥和其他凭据不进入仓库。
