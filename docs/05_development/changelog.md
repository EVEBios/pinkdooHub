...
## 2026-09-16 · Gate A M9→M15 迁移与收口

- 发布工具支持冻结 M15 候选的逐步迁移、完整内容快照、管理员验收、故障恢复、备份/独立恢复和最终发布记录；候选自己的九项 CI 与持久环境全链路验证通过。
- Gate A 已完成 M10–M15 六步迁移，current/live/database/finalized 一致；新增库存保持未盘点，保留存量业务数据及历史保护证据。详情与精确候选身份见[M15 完成报告](../09_release/reports/gatea_m15_completion_2026-09-16.md)。
- 本轮收尾不增加业务迁移、依赖或版本号；微信发布与其他环境仍须独立验收。

## 2026-09-15 · P2.3 桌台到时预警

- 根据用户选择在体验结束前 10 分钟逐组提醒，缓冲继续、按桌去重；管理员首页桌台入口合并有效待收款桌数，列表增加完整“全部桌台／需关注”筛选，详情突出对应计时组。
- 桌台摘要新增轻量 timers，沿用预加载与既有权限；可见页面单请求轮询、后台暂停、返回校准、身份隔离与断网旧数据提示。自然结束仍由服务端确认。
- 不增加数据库结构、依赖或版本号，不改资金流程、不启用微信推送。验证及平台边界见 [P2.3 检查记录](../08_frontend/qa/p2-table-warning-review.md)。
- 用户确认电脑端及 iPhone 本次预警检查正常，覆盖首页红圈、需关注、查看不清除及前后台恢复；反馈时尚未到缓冲/释放自然节点，后两项不记录手机验收通过。修正临时测试包漏注入环境配置导致的启动白屏，独立测试环境完成回收；整体规划同步本轮收口与低库存暂缓，无业务源码变更。

## 2026-09-15 · 预约跟进说明框可编辑状态

- 跟进 Textarea 明确传递布尔 disabled，与按钮的 H5 省略 false 属性处理分开；禁用期间不接受迟到输入事件，保护已有草稿。
- 新增读取失败后恢复编辑和保存回归，相关 7 项测试、TypeScript、定向 ESLint 通过；微信开发产物已更新。开发者工具控件赋值后保存可用，清空后禁用；随后用户确认 5 项手工测试均正确，说明填写、保存、角标消除和历史查看验收通过。修复验证时未提交测试跟进记录；后续用户测试记录保留，无结构或依赖变更。

## 2026-09-15 · P2.1 店休联系与 P2.2 过期预约跟进

- 管理员工作台增加“预约跟进”及共享待处理数字，提供店休联系／过期跟进、待处理／已完成筛选；在预约详情记录联系结果、必填说明、操作人及时间。待重试与号码不可用继续提醒，已联系或结束跟进按类别收口，查看、拨号和顾客已读不会完成任务。
- 新增独立的不可变跟进记录及 M14：预约锁、递增版本与精确请求键保障并发和重放，保留原预约状态；前端处理未知保存、旧草稿核对、断网及前后台恢复。两行筛选与表单双列按钮沿用既有设计。
- 后端完整 3224 通过／85 跳过；隔离 MySQL 从空库升级 M0–M14 后 85 通过；前端 117 套件／903 项通过。类型、ESLint、定向 Stylelint、生成契约与 H5 编译通过（保留原两项包体积警告）；320/390 H5 截图及独立设计复核通过。
- 新增迁移文件，未执行持久环境受控迁移；本地既有开发服务自动补建新表，不等同于 Aerich 迁移。无依赖／版本变更、提交或发布；P2 微信真机尚未验收，P2.3/P2.4/W 未实施。见 [P2.1/P2.2 检查记录](../08_frontend/qa/p2-followups-review.md)。

## 2026-09-14 · 订单详情桌台入口去重

- 同次桌台优先保留上方卡片入口，付款等结果提醒保留文字；卡片读取失败时保留备用入口，多条结果按会话去重，不影响其他历史会话访问或已读确认。
- 订单详情与结果提醒 3 套件/42 项测试通过，TypeScript、定向 ESLint 和差异检查通过；原有微信开发监听器已生成包含此次修改的产物。未新增依赖或迁移。

## 2026-09-14 · P1 联合验收与问题修复

- 修复 MySQL 并发抢桌沿用旧快照导致内部错误；锁后当前读返回明确占用结果，并保持订单、库存与桌台事务原子性。
- 修复 H5 身份恢复后会员中心白屏；购物车包装层稳定，按身份启用上下文。开启微信开发者工具正常代码压缩，解决开发包无法进入真机调试的问题。
- 新增跨模块存量/已读测试、MySQL 资金并发/晚提交测试、前端身份和前后台/断网生命周期测试；后端完整 3216 通过，真实 MySQL 77 通过，前端 115 套件/886 通过，四端编译完成。iPhone 15 Pro Max 现场确认登录、预约/资金已读、余额付款、桌台倒计时、前后台恢复及重登正常。
- 无新增迁移、版本、依赖或发布操作；手机现场与详细证据见 [P1 联合验收记录](../08_frontend/qa/p1-joint-review.md)。

## 2026-09-14 · P1.3 资金结果提醒

- 管理员代客钱包下单与两渠道退款接入订单结果，关联桌台结束合并为同一事项；独立余额调整按不可变流水逐笔提醒，成功事务内写入，幂等重放不重复。
- 顾客首页／会员中心增加余额变动提醒，会员 Tab 合并该数量；新增两类筛选的余额列表与精确详情，展示原始流水金额、前后余额、时间和完整原因后确认已读。资金明细保留结果入口。
- 新增两个只读端点、wallet_unread 汇总及可空流水白名单；复用 M13 结构和唯一事件键，无新迁移、历史回填、版本或依赖变更。需求、API、数据库语义/DBML、架构、产品与前端契约和生成类型已同步。
- 后端 3215 通过／63 跳过，前端 113 套件／882 项通过，类型、ESLint、相关 Stylelint、编译与差异检查通过。真实 MySQL 与微信真机仍列 P1.4 联合验收；没有真实资金操作、持久迁移、提交或发布。详见 [P1.3 检查记录](../08_frontend/qa/financial-attention-review.md)。

## 2026-09-14 · 顾客“本次桌台”倒计时

- 详情显示付款剩余时间，以及各时长体验组的体验／缓冲剩余时间，保留绝对截止和整桌预计结束时间。
- 基于现有服务端时间在本地每秒更新独立区域，沿用 5 秒状态查询并补充截止边界核对；到零等待服务端确认，后台暂停，恢复校准，结束停止。
- 增加时钟偏差、分组、截止失败、状态切换、前后台恢复、慢请求隔离和权限回归测试。需求同步更新；无 API、数据库迁移、版本或依赖变更。

## 2026-09-14 · 修复付款后“我的桌台”消失

- 会员“我的桌台”改为固定入口，提醒归零或汇总加载失败时仍可进入；红圈只统计原待办与未读事项。
- 新增本人全部开台记录分页查询，保留使用中及已读历史桌台，按精确会话打开详情。管理员提醒统计不变。
- 覆盖人工／余额付款、已读后可访问、结束后历史保留、身份隔离和到期边界；无需迁移或新增依赖。

## 2026-09-14 · 会员提醒入口 Option 2

- 会员中心订单／桌台提醒融入深莓页头，采用独立图标、两行文案、数字角标与箭头，保留整行跳转和 0／99+ 规则。
- 窄屏自然换行；钱包失败时保持深莓底与可读对比。首页提醒样式保持原状。
- 新增本地双椅桌台图标，按用户选定预览以 Image Gen 制作；无后端、接口、数据迁移或依赖变更。验证见 [设计检查](../08_frontend/qa/member-attention-review.md)。

## 2026-09-14 · 扫码与商城统一选桌结算

- 修复扫码选商品后进入购物车会丢失开台关联的问题。选桌按账号跨页面保存，商城确认页支持扫码/输入桌号；新开台订单在创建时原子绑定桌台，成功直接进入订单付款。
- 复用现有桌台会话保存新订单幂等事实，创建订单、明细、库存、占台与审计同事务提交。同键重放不重复建单/扣库存/延长占台。原有待付款体验订单也可在详情选桌。
- 钱包付款与管理员收款统一拒绝过期或已结束桌台，禁止静默普通付款；确认时携带精确会话。管理员订单页显示关联桌台并在收款前核对。
- 处理选桌失效、账号切换、未知提交恢复、成功后上下文消费及旧提醒快照重复已读。按钮沿用现有居中、加粗和显式间距样式。
- 后端全量 3203 项通过、63 项跳过；前端 110 套件/845 项通过；TypeScript、ESLint、新增样式 Stylelint、OpenAPI 一致性和差异检查通过。真实 MySQL 并发与微信真机付款未验收；具体范围见 [检查记录](../08_frontend/qa/table-checkout-review.md)。
- 已更新需求/API/架构/前端契约和生成类型。无需数据库迁移、依赖或版本变更，未提交、推送或发布；用户已有本地服务保持运行。

## 2026-09-14 · 结果提醒按钮对齐与字重统一

- “查看这次桌台”及共享提醒操作按钮复用项目次要按钮样式，使用 flex 双轴居中、统一按钮字号与 700 字重，并补充浅莓色边框；保留全宽和 12rpx 顶部间距。
- 定向 Stylelint 与 Sass 展开后的居中、字重、字号、高度和间距检查通过。仅共享提醒按钮样式调整，跳转、已读和重试逻辑不变；无接口、依赖或迁移变更。

## 2026-09-14 · 开台及近期新增页面按钮间距巡检修正

- 修复扫码开台次要按钮的 mixin 清零间距，覆盖继续挑选商品、刷新付款状态、查看订单与校准计时等入口；同步恢复管理员桌台列表和会话详情的收款按钮间距。
- 补齐订单／桌台提醒和历史桌台未登录提示与按钮的留白。检查 12 个相关页面及直接开台表单、共享提醒组件，其余按钮组间距正常。
- 定向 Stylelint、TypeScript、ESLint、layout 扫描、Sass 最终层叠及微信 WXSS 检查通过。Review 限定局部间距与 className，保留业务逻辑和既有改动；无依赖、接口或迁移变化，未发布。范围与验证边界见 [间距检查记录](../08_frontend/qa/table-action-spacing-review.md)。

## 2026-09-14 · 订单详情底部区块间距修正

- 将取消/返回按钮的页面间距规则移到按钮公共样式之后，避免公共 `margin: 0` 覆盖顶部留白；时间信息与“返回我的订单”之间恢复 18rpx 间距。
- 定向 Stylelint、Sass 编译后样式层叠检查及现有微信构建产物检查通过。仅局部样式调整，阅读顺序、按钮行为与其他页面不变；无依赖、接口或迁移变化，无新增长驻资源。

## 2026-09-14 · 支付结果触发订单详情反复加载修正

- 修复结果提醒调用整页重载导致自身卸载、重挂载后再次发现同一结果的循环。订单详情新增保留内容的后台刷新，结果通知改用该刷新；初次加载和主动整页重试继续沿用原路径。
- 后台读取失败保留订单快照与结果内容，明确提示尚未更新并提供订单/资金联合重试；同一结果不重复触发刷新，新完成结果仍正常更新并精确已读。
- 新增真实订单/支付/资金 Hook 与真实提醒组件组合测试，覆盖余额支付后的稳定页面、已读/未读历史结果、新结果更新及后台失败恢复。定向 35 项、前端全量 108 套件/828 项、TypeScript、定向 ESLint 和差异检查通过。
- 已检查用户现有微信 watch 构建产物包含后台刷新调用、实现与失败提示；未启动额外构建监听、未进行微信运行时操作。只读查询本地订单 10 为已支付且只有一条成功 Payment；本次无资金、数据库、API、依赖或版本变更，未发布。Review 核对刷新生命周期、请求序号与确认/取消原流程，临时测试日志已清理。

## 2026-09-14 · 本机 SQLite M13 提醒结构已应用，修复人工/余额支付缺列 500

- 用户明确授权后完成全模型结构审计：实际缺口仅提醒表 M13 字段、可空预约引用、外键及真实列索引；退款命名唯一索引与目标约束等价，保留原状。
- 余额支付同样因提醒结果缺列而事务回滚；副本复现并证明余额/流水不变，升级后余额支付及同键重放只产生一次支付和结果。人工收款已在此前副本演练通过。
- 停写、0600 一致性备份后应用本机 `db.sqlite3`，全部 28 张表原有数据保留；27 张模型表的结构语义匹配、完整性、外键、幂等重放及提醒读取检查通过。后端已恢复到 `127.0.0.1:8000`，数据库/Redis 就绪；无实际收款、扣款或发布。
- 本机备份、进程、恢复与环境边界见 [M13 实际应用记录](../07_process/database_migration_workflow.md#本机-m13-实际应用记录2026-09-14-1932asiashanghai)。旧条目中“待授权应用”为当时状态，由本条更新。

## 2026-09-14 · 本地收款 500 定位与 SQLite 提醒结构升级工具（待授权应用）

- 日志定位人工收款失败为本地 M12 提醒表缺失 M13 结果字段；失败事务已回滚，订单 1 仍待支付且无支付记录。新增默认只读的本地升级工具，应用须单独授权并停写，备份后单事务保留历史事件/已读、自增号与外键，修复 SQLite 已知表达式索引形态。
- 定向测试 20 项通过；在当前库可销毁副本复现错误、升级并逐表核对全部 28 张表原有字段/记录，再经真实服务验证收款与结果事件各一次。Review 已核对默认只读、备份、回滚、幂等、未知结构拒绝和数据保留；无依赖、版本或 MySQL 迁移文件变化。
- 当前持久数据库未改，服务未暂停，未执行实际收款或发布；操作与恢复方案见 [M13 本地升级说明](../07_process/database_migration_workflow.md#本地-sqlite-m12m13-工具与未应用演练2026-09-14)。

## 2026-09-14 · 微信收款确认弹窗按钮长度修正

- 修复点击“标记为已支付”后微信 `showModal` 因确认按钮超过四个汉字而失败：弹窗按钮改为“确认收款”；同类桌台收款和代客下单按钮改为“收款计时”“确认扣款”。页面入口名称与业务确认流程保留。
- 排查全部 23 个显式弹窗按钮文案，均不超过四个字符；补充三处长度断言及取消收款不更新状态的回归测试。相关 3 套件、24 项测试以及 TypeScript、定向 ESLint、差异检查通过。
- Review：改动限于前端按钮文案与回归测试；无业务接口、数据库、依赖或版本变化，未执行真实收款、微信运行时验收或发布。无需迁移，需前端重新构建后生效。

## 2026-09-14 · 订单与桌台站内交接 P1.2（M13）

- 管理员订单/桌台首页角标、待付款与履约列表完成仓库实现；有效占台订单只归桌台入口，退款订单从履约队列排除。
- 人工/余额收款、订单完成与关联桌台变化合并顾客结果；独立超时、到期和释放按会话计数。旧会话结果新增 owner-only 落点，查看后按精确事件已读，隐藏与身份切换隔离。
- 顾客商城、会员中心与会员 Tab 显示订单/桌台事项，沿用原视觉与直接展示筛选；原预约链路保留。
- M13 增量扩展事件表，不回填旧结果，有结果时禁止有损降级；无依赖或版本号变化。持久环境未迁移、未发布，P1.3 资金结果和微信仍待后续阶段。
- 验证、截图与限制见 [P1.2 验收记录](../08_frontend/qa/commerce-attention-review.md)。

## 2026-09-14 · 预约拒绝文案与筛选换行

- rejected 的状态名统一为“已拒绝”；预约详情改为“拒绝原因”，顾客结果和站内提醒明确门店已拒绝及无空位原因。同步服务端展示 Registry、前端严格协议校验和需求/API 文档。
- 管理预约六个筛选项使用三列两行等宽布局；顾客预约筛选同步三列换行，取消横向滚动。保留胶囊选中态与实际筛选行为。
- 不改变状态值、权限和持久数据，无迁移或依赖变更；前后端需同步加载新展示协议。验收见 [筛选修订检查](../08_frontend/qa/reservation-filter-review.md)。

## 2026-09-14 · 预约待办视觉统一与过期展示修正

- 待办页沿用现有管理端纹理页头、圆角筛选、胶囊选项与预约卡片；空状态提供可用下一步，初次进入优先非空集合。
- 修复原管理列表仍把过期 pending 显示为待确认：新增服务端 review_timing 分页筛选，前端待确认/已过期分开；顾客与管理列表/详情显示已过期未处理，管理详情收起过期审核操作。
- 不改历史业务状态，无新增迁移或依赖；原 API 省略新筛选参数时兼容。验证与图片见 [设计检查](../08_frontend/qa/attention-refinement-review.md)。

## 2026-09-14 · 预约站内提醒 P1.1（M12，待目标环境迁移验收）

- 管理员首页预约入口显示可审核/取消变更数字；顾客首页、预约页和预约 Tab 显示未读结果，新增独立分页提醒视图。
- 确认、拒绝、单日/固定店休取消与用户结果同事务；顾客取消已确认预约生成门店共享变更，管理员点击“已知悉”后共享消除。到开始时间的 pending 单列过时，不再计可处理。
- 精确预约版本与事件 ID 回执避免旧详情清除新结果；按身份清空状态、前台轮询、合并请求、错误保留旧数并可重试。
- M12 追加 attention_events，不回填历史结果，有数据时拒绝降级。新增提醒协议与生成类型，原预约协议不变，无新依赖或版本号变更。订单/桌台/资金提醒和微信投递仍为后续阶段。
- 测试、构建、截图及限制见 [P1.1 页面检查](../08_frontend/qa/attention-review.md)。仅验证仓库与隔离环境，未迁移持久数据库、未上传或发布。


## 2026-09-14 管理桌台计时与直接开台（M11，待目标环境迁移验收）

- 30 桌管理列表显示体验/缓冲/付款等待剩余时间；管理详情显示各组计时，停用新开台仍保留活动会话展示。
- 待付款桌及会话详情可确认整单线下收款并激活计时，复用人工 Payment/Settlement，精确绑定当前会话；顾客页面自动同步。
- ADMIN+ 可为无账号、无系统订单的顾客直接开台，时长来自有效体验配置，保存快照并沿用 10 分钟缓冲、唯一占用、幂等、审计和自动释放；不生成销售或资金记录。
- 新增 M11 MySQL 迁移和默认只读的本地 SQLite 升级工具；现有二维码映射保持不变。仓库实现不代表 Gate A 或任何持久环境通过迁移与验收。

## 2026-09-14 · 空购物车可见区域居中

- 修正购物车状态页按整屏居中造成的偏移：小程序状态页恢复浮动底栏避让；H5 按扣除原生 Tab 高度后的可见区域居中标题、说明和按钮。
- 保留有商品时的列表及结算布局。购物车 13 项测试、定向 Stylelint 通过；H5 在 320／390 宽度测量内容组合中心与可见区域中心一致。

## 2026-09-14 · 管理订单输入框提示截断修正

- 管理订单的商品名称、订单号和用户 ID 输入框改用块布局与明确高度，避免原生提示层被 flex 布局收缩；H5 内层输入同步占满可用宽度。
- 保留完整占位文案、筛选行为与现有配色。管理订单页面 5 项测试及定向 Stylelint 通过；小程序实机显示仍需复核。无 API、依赖或迁移变更。

## 2026-09-13 · 全页面轻量点缀巡检

- 检查全部 36 个注册页面；17 页补充浅粉标题带、柔莓细线或信息底色，19 页保持既有层次。修复管理筛选区强调线被阴影覆盖，统一桌台卡片和长编号排版，增强顾客支付方式名称对比。
- 本轮仅改 14 个 SCSS 文件。Stylelint、TypeScript、ESLint、H5／微信 CI 构建及产物检查通过；修改页在 320／390／768 宽度复核。全量前端测试 793／794 通过，1 项为本轮未修改的会员头像旧偏移断言不一致。
- 记录与截图见 [全页面巡检](../08_frontend/qa/all-pages-accent-review.md)。无 API、迁移、版本或依赖变更，未发布。

## 2026-09-13 · 管理订单详情轻量点缀

- 为下单用户／商品卡增加柔粉细边线，小计增加淡粉底，时间与备注采用浅粉实色；资金标题增加浅粉分区与窄莓红顶线，让页面下部更有层次。
- 仅调整详情页样式，保留页面结构、文案、退款与状态切换逻辑。相关14项测试、Stylelint、H5／微信CI构建与320／390／768宽度视觉复核通过；没有API、依赖、迁移或版本变更。


## 2026-09-13 · 修正会员余额卡排版与钱包比例

- 根据实际截图，将未开通充值从高按钮改为紧凑状态说明，收拢左侧三行，保持资金明细触控入口；仅服务端开通且账户可用时显示充值按钮。
- 钱包改用正方形容器与等比显示，图案／金额组合居中，零余额到上限金额保持单行。保留现有钱包图案和莓红配色。
- 会员与钱包相关 20 项测试、类型／静态检查、H5 与微信 CI 构建通过；隔离浏览器确认 320／390 宽度及三种余额，验收记录见 `docs/08_frontend/qa/balance-layout-fix.md`。无依赖、API 或迁移变更。

## 2026-09-13 · 会员中心 Option 4 精修

- 按最终确认稿将身份与订单合并为连续纹理区，三项订单入口使用白色轮廓图标与透明描边；余额在订单下方，保留定制钱包图案并调整为柔莓玫瑰色。
- 个人资料完整展开，按身份／账户字段分组，加入日期简化展示；退出改为浅粉胶囊，修复 H5 正常态 `disabled="false"` 导致误套禁用配色的问题。
- 保留真实钱包状态、资金明细／充值能力说明、订单筛选、头像回退及顾客导航。无 API、数据库、依赖或版本变更；验收见 `docs/08_frontend/qa/member-refinement-review.md`。

## 2026-09-13 · 购物车导航与会员订单专区

- 第三个顾客根导航由订单调整为购物车；会员中心按选定设计增加订单专区、全部订单与待支付／已支付／已完成快捷入口，收拢资料详情。
- 订单列表成为普通二级页，保留现有筛选与回会员中心入口；同步登录回跳、下单结果、订单详情、扫码开台、商品详情路由，以及微信构建产物检查。
- 钱包加载失败仍可查看订单；购物车结算区预留底栏空间。无后端 API、数据库迁移或运行时依赖变更。
...
## 2026-09-13 · 我的预约创建入口视觉调整

- 按 Product Design 展示顺序 Option 2，将“新建预约”从纹理页头右侧移到标题下独立行动区；使用莓红胶囊按钮与 Heroicons 加号，左侧说明“到店时间先约好／体验项目可以到店选”。
- 空列表复用同一入口；保留筛选、预约卡片、服务端文案、分页及根导航。没有 API、数据库或依赖变更。
- 103 个前端测试套件、775 项测试通过；类型、定向 ESLint／Stylelint、H5 构建和 320／390／768 宽度浏览器验收通过。视觉证据见根目录 design-qa.md 对应条目；浏览器接口使用隔离 fixture，不作为真实下单或微信真机验收证据。

## 2026-09-13 · 预约支持可选套餐

- “我的预约”新增创建入口；新建页优先选择日期和开始时间，完整体验套餐作为一组选填，沿用现有 Ribbon Ledger 和扫码开台商品展示。
...
- 顾客扫码页按 Product Design Option 3 重排为“扫码后选商品”：读取到可用桌台后可直接浏览并选择 Experience，也可同单添加材料包，不再要求先建单再扫码；新建含 Experience 的订单成功后会自动返回原桌台上下文，由顾客确认后绑定桌台并进入待支付状态。计时仍只在服务端确认支付成功后开始。本轮仅更新仓库内顾客流程，继续使用普通占位二维码与本地/`develop` 内部测试边界，不代表微信官方小程序码、真实微信支付或生产环境部署已经完成。
- 2026-09-13 修复旧订单回跳链接的误导性空态：聚焦订单不在可开台列表时，先读取该订单的当前状态；已付款/已完成/已取消的历史订单提供重新选择商品入口，已结束的桌台会话明确显示“上次体验已结束”，并清除旧聚焦路由。仍为 Pending 但不可开台或状态读取失败时继续阻止盲目重复下单，只允许重新核对或查看我的订单。该改动只更新顾客小程序呈现与恢复路径，不改变订单或桌台服务端资格规则。
- 2026-09-13 在顾客扫码开台页的共用页头加入右上角“返回商城首页”房子图标，覆盖读取、异常、选品、待支付、计时中及已结束等状态；入口仅切换到商城根 Tab，不取消订单、不清除待处理开台意图，也不改变服务端计时。图标沿用 Heroicons v2.2.0 的 MIT 授权资源。计时中加购与续时仍未实现。
...
### 2026-09-15 — P2.4 门店用豆库存（候选实现）

- 管理首页增加“用豆库存”普通入口；221 色双列文本台账、库存状态/颜色组两维筛选、未盘点与零库存区分。编辑显示包数与加减，草稿跨筛选保留，完整变更清单二次确认后原子保存。
- 独立整包余额、调整批次和逐色历史；版本冲突不覆盖、未知请求核查与幂等重放、管理员权限及事务内审计。库存不足仅筛选，不创建红点、红边、提醒事件或推送，不联动商城 10g 库存。
- 新增 M15 三表迁移与非空拒绝降级，OpenAPI/类型、CI 隔离迁移链与文档同步；未运行持久迁移命令，本地 development 只读现场与元数据差异另记于 QA。
- 后端全量 3234 passed；前端全量 939 passed；微信/支付宝/抖音/H5 构建与微信体积检查通过。独立视觉复核 F1–F3 全部 resolved；真实 MySQL 门槛未跑、真实手机由用户后续验收。[完整交付记录](../08_frontend/qa/store-bead-stock-review.md)。

### 2026-09-15 — 修复用豆库存微信 WXSS 编译失败

- 将页头结构伪类与页脚通配符替换为明确的类选择器，保留标题伸缩与确认按钮间距。
- 使用微信开发者工具自带 `wcsc` 复现并验证实际页面/依赖 WXSS 编译通过；增加静态与可选原生编译回归，模块 11 项通过。详见[验收记录](../08_frontend/qa/store-bead-stock-review.md)。

### 2026-09-15 — 用豆库存「柔粉分区」视觉修订

- 按用户选定的 Product Design Option 3 调整浏览、编辑、二次确认：浅粉双维筛选、双列隔行台账、紧凑原因／备注与莓果主操作。0/1 包继续保持普通样式，不引入绿色、黄色或库存提醒。
- 二次确认及记录详情保留用户指定的“左侧增减包数 + 右侧修改前 → 修改后”；360px 及以下将数字输入独立成行，保留完整数字和加减触控范围。
- 增加确认清单增减／前后数量共存与返回保留草稿的回归。验证与实屏见[本轮视觉检查](../08_frontend/qa/store-bead-stock-soft/review.md)。无数据库、接口、依赖或版本变更，未执行迁移或发布。

### 2026-09-15 — 修复用豆库存页头按钮文字偏上

- “调整库存／取消调整”改为复用既有 `button-base`，显式双向居中、重置原生行高和盒模型；不靠空白、单侧内边距或文字位移补偿。
- 在 DESIGN 的 Buttons 章节记录成因与强制规则，前端架构和评审清单引用同一规范。编译后的按钮布局契约增加回归守卫，模块3组13项通过；TypeScript、局部ESLint、Stylelint、实际微信页面及依赖WXSS编译通过。
- 独立浏览器以Taro宿主元素和原生行高进行16组布局验证：修复后文字框横纵中心最大偏差0px，旧行为模拟最大纵向偏差18px。该验证不代替微信真机实屏确认。证据见[按钮检查](../08_frontend/qa/store-bead-stock-soft/button-centering.png)。无接口、数据库、依赖或迁移变更。
