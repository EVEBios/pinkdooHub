...
本次仅增加预约 Tab（索引 1）的结果数量镜像，与商城/预约页提醒入口共享身份隔离状态。0 隐藏、99+ 扩宽，标签和图标仍是完整触控目标，不新增消息 Tab。微信自定义底栏订阅汇总，其他兼容端使用原生角标；退出清空、旧身份迟到响应忽略。当前四根页名称与路由以 PRODUCT.md 和 app.config.ts 为准；上文历史订单 Tab 描述不作为本次新增导航依据。

证据见 docs/08_frontend/qa/attention-review.md；H5 兼容检查不替代微信实机检查。
