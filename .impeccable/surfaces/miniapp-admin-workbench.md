...
ADMIN 与 SUPER_ADMIN 的独立店铺工作台。它整理八条管理链路；P1 汇总预约、订单与桌台待办，P2 独立入口汇总店休待联系与过期待跟进，P2.3 在桌台入口合并有效待收款与到时桌数。
...
STORY: 先确认当前工作人员身份，再按时效从“待处理”进入预约或订单；商品、库存、营业日历与用户权限按业务关系继续下沉。完成子任务后通过系统返回回到同一目录，最后从安静的退出动作结束会话。

FIRST VIEWPORT: 紧凑的深莓／李子色页头展示“店铺工作台”、品牌、昵称／用户名／角色和一句工作提示；标题、品牌和身份使用白色，副标题使用近白粉 `#fbeef3`。首个“待处理”瓷白账簿紧随其后，让预约审核和订单处理在手机首屏优先可见。
...
| 分组 | 入口 | 辅助文案 | 目标路径 |
|------|------|----------|----------|
| 待处理 | 预约审核 | 待审核 N · 取消变更 M | `/pages/attention/index`，不可用时完整预约列表 |
| 待处理 | 订单处理 | 支付确认与履约 | `/admin/pages/orders/index` |
| 待处理 | 预约跟进 | 店休待联系 N · 过期待跟进 M | `/admin/pages/reservation-followups/index` |
...
| 门店与权限 | 桌台工作台 | 待收款 N · 到时提醒 M；数据不可用时保留原入口说明 | `/admin/pages/tables/index` |
| 门店与权限 | 用户与权限 | 账号状态与资金入口 | `/admin/pages/users/index` |

七项入口对应既有业务页面。预约汇总可用时进入 `/pages/attention/index`，副文案显示待审核与取消变更；不可用时保留完整预约列表入口与刷新提示。其他入口保持原路由，不以多个列表第一页拼凑数量。
...
- Guest 在工作台只看到登录引导；普通 USER 只看到无权限状态和返回商城；认证初始化、错误或用户资料缺失时 fail closed，不展示七项入口。
...
- 三个分组继续共用一个瓷白外轮廓和内部实色分隔，不把七项拆成七个同权悬浮卡片。每组先铺一整块分组强调色，再由同一裁切容器形成顶部与标题区左侧连续的 L 形装饰脊线，避免在圆角转角处拼接两条边框：待处理使用深李子 `#68213f`，商品与库存使用核心莓果 `var(--pd-color-primary)`（回退 `#b7355d`），门店与权限使用灰粉 `#c97991`。
...
- 工作行显式使用自定义 `hoverClass` 覆盖小程序原生整块变暗反馈：按压态只改变“进入”胶囊，行背景和左右文案保持原样。管理跳转处理中不得给完整动作行设置原生 `disabled`；当前行只增加专用 pending class，并由它把“进入”胶囊切换为柔莓墨、浅粉灰底、无阴影的静止状态，其余区域及其余入口保持原有对比度。并发导航仍由独立引用锁拦截，防止在原生导航尚未结束时发起第二次跳转。退出处理中则只禁用退出动作。
- 手机宽度下各组入口纵向排列；`768px` 起变成两列，并把第二项的上分隔线转换为左分隔线。页面本身不得横向滚动。
...
- 检查七项分组、顺序、文案和 URL，确认视觉精修没有改变整行触控、`navigateTo` 行为或任何路由，并且没有顾客钱包、商品购买入口、虚假数字、图标或顾客／管理员底栏。
...
## P1.1 finish review（2026-09-14）

预约标题旁增加红底白字数字，整行触控和去向胶囊保留。0 隐藏，99+ 扩宽，副文案明确数量含义；汇总失败保留原业务入口。390/768 H5 页面与操作证据见 docs/08_frontend/qa/attention-review.md，微信实机未验证。

## P2.3 finish review（2026-09-15）

桌台入口沿用标题旁红底白字数字和完整账簿行触控；数字合并同一桌台快照中的有效待收款桌和到时桌，按桌去重。辅助文案明确显示“待收款 N · 到时提醒 M”，0 时隐藏红圈并保留数量说明。点击仍进入完整桌台工作台，由桌台页的“全部桌台／需关注”按钮筛选，不新增入口、分组或已读动作。

到时提醒包含具体组临近体验结束与缓冲阶段，以及已走完计时、仍待服务端确认结束的活动桌。阈值、付款有效期与服务端状态边界以 [桌台需求 P2.3](../../docs/01_requirements/table_session_module.md#p23-管理员桌台到时预警2026-09-15) 为准；入口只是呈现数量，具体组与剩余时间在 [桌台操作 surface](miniapp-admin-table-operations.md) 展开。

独立 finish review disposition 为 `ship`。[workbench-320.png](../../docs/08_frontend/qa/p2-table-warning/workbench-320.png)、[workbench-390.png](../../docs/08_frontend/qa/p2-table-warning/workbench-390.png)、[workbench-768.png](../../docs/08_frontend/qa/p2-table-warning/workbench-768.png) 展示桌台入口合并红圈及分类数量，三组账簿、去向胶囊和页头身份层级保持既有实现。其余列表、筛选与详情证据合计组成 12 张截图，见 [P2 桌台预警 QA](../../docs/08_frontend/qa/p2-table-warning-review.md)。证据来自隔离模拟 API 的 H5，不构成微信真机、真实资金、持久环境迁移或发布验收。

本轮继承 Ribbon Ledger／Operate 和既有颜色、字体及按钮规则；不更新根目录 `DESIGN.md`、`PRODUCT.md` 或设计 sidecar，不将模拟数量与局部提醒布局确立为全局规则。
