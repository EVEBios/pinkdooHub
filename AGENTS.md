...
当前代码版本候选为 **v0.6.0（尚未发布）**；**Phase 4.1：Product Module**、**Phase 4.2：Order Module**、**Phase 4.3：Inventory Module**、**Reservation N1**、**M6 自选颜色 Kit** 与 **M7 可配置每周固定店休** 均已完成仓库实现。
...
- MySQL 8+ 权威 M0–M7 迁移链均已离线生成并通过静态契约测试；本地提交 `58d8435` 对应候选已在一次性 MySQL 8.0.46 真实执行完整 Aerich 0→7 与各历史起点重放，完成 M6/M7 结构快照及 Inventory+Reservation `21 passed`。实例已销毁，未使用 `--fake`；包含该修复的 head `4d6430c...` 已在远端 Run 34129910349 取得 8/8。任何迁移仍未应用持久、共享或生产数据库。
...
- Phase 4.3.12 最终 Review 已完成：分层、事务所有权、稳定锁序、MySQL 瞬态重试、幂等/隐私、Schema/OpenAPI、Mapper、Model/迁移/索引及文档联动均已复核；Product 用户/管理 Kit 详情响应补齐 `stock <= 999999`，数据库设计与 DBML 的旧“未来 Kit”描述已更新为当前 Kit/混合订单语义。全新隔离 MySQL 8.0.46 再次真实迁移并通过 9 项门禁，SQLite/MySQL 同进程完整套件 1431 项通过。代码候选收口为 v0.6.0；分支已 push，但未 tag/release，也未执行持久数据库迁移。
...
- Wallet/Payment/Refund v1 已完成仓库实现与扩展 MySQL 本地候选门槛；新 workflow 远端复现、M4 持久迁移、历史数据补齐和生产开关仍属于发布前工作。
- Reservation N1 已完成；N2 微信主动通知、可逆加密投递地址、durable outbox、worker、重试与监控仍是明确后续范围，不能把页面状态展示误报为主动通知。
- M7 之后的下一业务 Phase 尚未冻结；未经当前任务明确要求，不提前实现未来能力，不把规划误报为已完成能力。

当前仍为 **No-Go**：Gate A 持久环境在 2026-09-02 的最后留证为非空 M2，当前真实状态尚未重新只读确认；现有 `initial-migrate` 只支持空库，尚无经批准的 M2→M7 非空升级入口。MARD 色板仍缺持久 MySQL/对象存储导入流程；Wallet-expanded workflow 尚未远端复现，M4/backfill/reconcile 尚未对目标环境完成；旧 M7 head 的远端 8/8 不能替代新资金门槛，真实 Origin/RC、iOS/Android 真机及微信外部条件也未验收。其他已知限制包括真实微信支付 Provider/商户进件未接入、生产色板未经实体拼豆样本校色、Reservation N2 主动通知未实现、邮件验证与 OAuth、管理员启用用户及头像上传仍待实现。M4–M7 均未应用 Gate A/共享/预发布/生产 MySQL；Refresh family 轮换与认证限流已由 Phase 9.5 实现。不要在无关任务中顺手扩展这些范围。
