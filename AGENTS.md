...
- M9 当前 A→B→C 阻塞恢复候选的本地门槛为后端完整 `3130 passed, 39 skipped`、Release 完整 `1008 passed`；新增覆盖 verifier schema v2、schema v3 三重绑定 stage、双 pending typed allowance、双原始证据归档、八阶段崩溃恢复，以及 M9→M9 零迁移 adoption/replay。前端最近完整结果仍为 `102 suites / 719 tests`，TypeScript/ESLint/Stylelint/CI policy/OpenAPI 类型均通过；一次性 MySQL 8.0.46 的 M0→M9 迁移和 M9 发布门槛仍为 `39 passed`。这些本地与旧远端证据都不能替代全新候选 C 自身干净 SHA 的远端 9/9。Gate A/容量 Runtime 已关闭未经脱敏的 Uvicorn access log，Nginx 与异常日志对桌台 Token 路径统一脱敏；reconcile 同时核验打开和已关闭会话的付款/计时历史。微信 CI 使用独立 `.env.ci`，避免生产占位 Origin 覆盖 CI 注入值；页头纹理由全局 CSS 变量只内联一次，当前不可发布微信产物主包约 0.86 MiB、总包约 1.42 MiB。
...
- B `ad2ac8c1eb6633daebf3d66d9b3669e441c3c6dd` 已完成 stage，但其失败验收退休 journal 只到 `prepared`，尚未进入停写或 live verification；该 pending 是新的受保护阻断器，不能手工删除、修改或冒充成功。全新候选 C 的 stage schema v3 必须同时绑定 A failure digest、B stage Record digest 与 B prepared pending digest；C 的 takeover retirement 再按 `prepared` → `write-free-verified` → `superseded-retirement-archived` → `acceptance-archived` → `record-published` → `superseded-retirement-removed` → `canonical-removed` → `runtime-restored` 八阶段收口。B 原 journal 与 A 原 acceptance pending 分别进入 `/srv/pinkdoohub/gatea/records/m9-retirement-failures` 和 `/srv/pinkdoohub/gatea/records/m9-acceptance-failures`；两个目录都必须是 guarded `root:root 0700`，归档文件保持 `root:root 0600`、`nlink=1`、稳定 digest，并作为后续 activation/rollback/adoption/finalize 的持续 lineage 依赖。
- takeover verifier 使用全新 C 镜像，只通过有界 stdin 接收冻结 ID、A acceptance attempt ID 与候选身份，真实进程 argv 必须为空且 schema version 必须是精确整数；它要求同一 Kit 的全部库存流水恰为 SUPER_ADMIN supply `0→10`、订单扣减 `10→9` 和取消恢复 `9→10` 三条，拒绝缺失、额外或字段漂移，并确认该 Order 没有 Payment/Settlement/Refund/WalletTransaction/TableSession。takeover 恢复 allowance 必须把 C 当前 pending、B superseded pending 与 A acceptance sidecar 交叉绑定到同一 A runtime 和精确 checkpoint；普通 work/control error 优先于恢复阶段延迟信号传播。
...
当前仍为 **No-Go**：最后已 finalized 的 lineage 是 M7 Runtime S `73dca350505d43775fb1ff1158ccf6aabc221998`，live config、数据库和五服务是旧候选 A/M9 `d6c09482ee0f5583d79bd847e995746c9c6ee1a3`，B `ad2ac8c1eb6633daebf3d66d9b3669e441c3c6dd` 只留下 `prepared` retirement pending。唯一允许的后续是全新候选 C 先取得自身干净 SHA 的远端 required Jobs 9/9，再执行 schema v3 `stage(C, A failure digest, B stage/pending digests)` → 八阶段 takeover retirement → 全新 A/M9 source Backup/Restore → `activate-config` A→C → M9→M9 零迁移 adoption/replay → `app-up` → C acceptance → 显式绑定 acceptance 的 resilience → C 数据后 Backup/Restore → 以 source=S、predecessor=A、target=C `finalize`。禁止手工删除任一 pending、复用 B 或旧 Run 的 CI、临时注入 Secret、重跑 M7→M9/M8/M9 迁移、降级数据库或拼接旧证据。Gate A SSH 目标保持 `ubuntu@118.195.195.59`。真实 Origin、`release_eligible=true` RC、iOS/Android 真机、微信官方小程序码、upload/gray/release 与真实微信支付仍未完成或未授权；共享、预发布和生产环境未触碰。
