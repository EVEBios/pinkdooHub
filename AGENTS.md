...
- M9 当前修复候选的本地门槛为后端完整 `3067 passed, 39 skipped`、Release 完整 `945 passed`；其中新增覆盖 A→B 失败验收退休、M9→M9 零迁移接管、M8 色块内容摘要、跨候选恢复 allowance、订单钱包流水/完整库存审计链与恢复错误优先级。前端最近完整结果仍为 `102 suites / 719 tests`，TypeScript/ESLint/Stylelint/CI policy/OpenAPI 类型均通过；一次性 MySQL 8.0.46 的 M0→M9 迁移和 M9 发布门槛为 `39 passed`。这些均为本地或一次性环境证据，不能替代 B 自身的全新远端 9/9。Gate A/容量 Runtime 已关闭未经脱敏的 Uvicorn access log，Nginx 与异常日志对桌台 Token 路径统一脱敏；reconcile 同时核验打开和已关闭会话的付款/计时历史。微信 CI 使用独立 `.env.ci`，避免生产占位 Origin 覆盖 CI 注入值；页头纹理由全局 CSS 变量只内联一次，当前不可发布微信产物主包约 0.86 MiB、总包约 1.42 MiB。
...
- 当前候选新增受控 M9→M9 adoption：B stage 精确绑定 predecessor A、lineage S 与旧 pending digest；`retire-failed-acceptance` 先创建 `prepared` journal，再计划停止 App/Nginx/`table-sweeper`，在 MySQL/Redis 健康且三个写入方持续停止的窗口中，于每次数据库快照、wallet/table reconcile 和 B 镜像只读 verifier 前后重复断言 A/S 身份与停写状态。它按 `prepared` → `write-free-verified` → `acceptance-archived` → `record-published` → `canonical-removed` → `runtime-restored` 六阶段 no-clobber 收口；无论 work/control error 都进入独立进程组的强制恢复并复验 A 的五服务，recovery/cleanup failure 优先于原错误，延迟信号最后传播。原始 acceptance archive 是后续 `activate-config`、`rollback-config` 和 `finalize` 的持续 lineage 依赖，必须留在同一 guarded `0700` 目录中并保持精确路径、`root:root 0600`、单链接稳定文件、digest、attempt/source/image 绑定，不能在 retirement 后移动、删除或替换。随后只允许基于全新 A/M9 Backup/Restore 切换 A→B 配置，并以 schema v2 Record/evidence/replay 证明 `database_changes_applied=false`、`migrations_applied=[]`；不得重跑迁移、MARD、Wallet 补齐、30 桌 bootstrap 或修复写入。桌台身份读取已改为保留默认 Entrypoint 的一次性 `docker compose run --rm --no-deps app`。新 M8/M9 Backup 另以 `m8-swatch-content-v1` 按 `bead_colors.id` 稳定绑定 `id/slot_no/swatch_hex`；adoption、M9 resilience 与 finalize 必须连同 M7 保留、M8 色块、M9 桌台三份内容摘要和图片 manifest 一起证明零漂移。历史 M8/M9 Backup/Restore 若三项 swatch 字段全部缺失仍可兼容读取/异机导出，但任何部分出现或不匹配都拒绝，且这类历史证据不得进入本次 A→B 链。该恢复能力仍须由 B 自身新 CI 9/9 后才可在现场执行。
- 失败验收退休的 B 镜像 verifier 只通过有界 stdin 接收冻结 ID，真实进程 argv 必须为空且 schema version 必须是精确整数；它要求同一 Kit 的全部库存流水恰为 SUPER_ADMIN supply `0→10`、订单扣减 `10→9` 和取消恢复 `9→10` 三条，拒绝缺失、额外或字段漂移，并确认该 Order 没有 Payment/Settlement/Refund/WalletTransaction/TableSession。`app-up` 的 candidate-transition 与 acceptance-sidecar 两个 own-recovery allowance 必须交叉绑定同一 runtime candidate；普通 work/control error 优先于恢复阶段延迟信号传播。
...
当前仍为 **No-Go**：最后已 finalized 的 lineage 是 M7 Runtime S `73dca350...`，但 live config、数据库和五服务已经是旧候选 A/M9；A acceptance 在开台前的 `order_created` 检查点失败，自动清理完成且失败 pending 保留。唯一允许的后续是全新候选 B 先取得自身干净 SHA 的远端 required Jobs 9/9，再依次执行 `stage(B, predecessor=A, pending digest)` → 只读核验并耐久退休失败证据 → 全新 A/M9 source Backup/Restore → `activate-config` A→B → M9→M9 零迁移 adoption/replay → `app-up` → B acceptance → 显式绑定 acceptance 的 resilience → B 数据后 Backup/Restore → 以 source=S、predecessor=A、target=B `finalize`。禁止手工删除 pending、临时注入 Secret、重跑 M7→M9、降级数据库或拼接旧 Run 证据。真实 Origin、`release_eligible=true` RC、iOS/Android 真机、微信官方小程序码、upload/gray/release 与真实微信支付仍未完成或未授权；共享、预发布和生产环境未触碰。
