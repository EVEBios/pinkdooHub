...
> **2026-09-15 E 验收失败更新：** E 已完成自身 CI 与 adoption，但管理员辅助验收因认证 username、Nginx 双写原始 QR URL 而失败；真实付款与已关闭会话必须保留。当前新候选的四项修复、受控 paid recovery 及验证边界以 [已付款恢复记录](docs/09_release/reports/gatea_m9_paid_recovery_2026-09-15.md) 为准，以下较早的 C/D/E 待执行叙述属于历史；Gate A 仍为 No-Go。
...
- M6 自选颜色 Kit 已完成仓库实现与本地回归：全局 221 个 BeadColor 槽、商品级 ProductKitColor 启用/10g 库存、统一每 10g 价格、每单最多 20 个颜色且每色最多 990g、颜色订单快照、扣减/取消/PAID 退款恢复、管理 API 与小程序页面均已接入；既有 fixed Kit 保持兼容。2026-09-07 已冻结 MARD A1–M15 共 221 色 manifest，并在本地 SQLite 导入 code/name/URL、确定性 PNG；2026-09-08 持久 Gate A 又在 M6 阶段发布 221 色元数据/PNG、建立三启用色测试商品和库存，随后完成 M7 数据后 Backup/Restore。M8 再把 `swatch_hex` 纳入正式字段；live Gate A 已由旧候选 A 带入 M9，但 `current` 和最后已 finalized lineage 仍是 S/M7，A 尚未完成验收与收口。现有 PNG 只作兼容回退，不转 WebP、不删除。
...
- MySQL 8+ 权威迁移链现为 M0–M9，均已离线生成并通过静态契约测试；M9 新增固定桌台、会话、计时器与占用四表。持久 Gate A 已于 2026-09-08 从经只读确认的 M2 受控应用 M3–M7，旧候选 A 又于 2026-09-11 实际执行 M8/M9，使 live config、数据库和五服务达到 A/M9；但 `current` 与最后已 finalized lineage 仍是 S/M7，且 A acceptance 失败，所以这不是 M9 PASS、finalized 或 release。共享、预发布和生产数据库不因 Gate A 证据自动迁移，仍须分别核验和授权。
...
- Phase 4.3.4 Inventory MySQL 8+ 增量迁移已离线生成并通过静态 Review，新增流水表并为 `stock > 0` 的现有 Kit 写幂等 `opening_balance`；零库存不写，余额不修改。完整链、正/零库存回填和 Phase 4.3.5 Repository smoke 已在一次性 MySQL 8.0.46 通过；该 M2 已在 Gate A 的 M0–M7 阶段应用并由当前 live M0–M9 继续继承，其他持久环境仍未因此自动应用。DDL 与数据回填因 MySQL 隐式提交不具备整体原子性，任何新目标环境执行前仍必须停写、扫描库存范围和备份。
...
- M9 A→B→C 阻塞恢复实现的最近完整本地门槛为后端 `3130 passed, 39 skipped`、Release `1008 passed`；新增覆盖 verifier schema v2、schema v3 三重绑定 stage、双 pending typed allowance、双原始证据归档、八阶段崩溃恢复，以及 M9→M9 零迁移 adoption/replay。前端最近完整结果仍为 `102 suites / 719 tests`，TypeScript/ESLint/Stylelint/CI policy/OpenAPI 类型均通过；一次性 MySQL 8.0.46 的 M0→M9 迁移和 M9 发布门槛仍为 `39 passed`。候选 C head `e909c42cebaf59931536ddc2c82a43f19a29925c` / merge target `c709d6252a07d65eb1457b23e7036ecb736f18b8` 已由 Run 34616037853 完成远端 9/9，但其真实 Gate A stage 在任何 C pending/Image/Release 写入前失败，不能作为已 stage 或已接管证据。Gate A/容量 Runtime 已关闭未经脱敏的 Uvicorn access log，Nginx 与异常日志对桌台 Token 路径统一脱敏；reconcile 同时核验打开和已关闭会话的付款/计时历史。微信 CI 使用独立 `.env.ci`，避免生产占位 Origin 覆盖 CI 注入值；页头纹理由全局 CSS 变量只内联一次，当前不可发布微信产物主包约 0.86 MiB、总包约 1.42 MiB。
- Gate A Backup 按精确 Aerich 链恢复常驻服务：M7/M8 只恢复 App/Nginx，不启动尚无桌台表的 M9 sweeper；M9 则要求 `table-sweeper` 备份前健康、与 App/Nginx 一起停写并在备份后严格恢复。当前工具新建的精确 M8/M9 Backup/Restore 必须以 `m8-swatch-content-v1` 按 `bead_colors.id` 稳定绑定 `id/slot_no/swatch_hex`；精确 M9 数据后 Backup/Restore 另以 `m9-table-business-v1` 按稳定主键顺序覆盖桌台、Session、Timer 和 Occupancy 四表。Record/CI Artifact 只保存摘要而不保存原始 Token/行内容，异机副本 export 与解密 verify 均按版本重验摘要；历史 all-absent swatch pair 的兼容范围以接管条目为准。普通 `app-up` 仍默认要求 M9 sweeper，未知链或停写窗口内版本变化 fail closed。
- M9 持久发布工具采用可恢复状态机：candidate `stage`/`activate-config`/`rollback-config`/`retire-failed-acceptance`/`finalize` 分开安装、配置、迁移前回退、失败验收退休与最终 `current` 切换。唯一 operation lock 之外，还会跨所有候选 SHA/所有相关入口扫描五类 candidate pending 和 M9 acceptance pending/complete；畸形名称、symlink/目录/FIFO 或第二 blocker 都 fail closed，只有原动作严格绑定 candidate/kind/path/checkpoint 的 own recovery 可获结构化 allowance。该 inventory 已接入 candidate、Backup/Restore、upgrade、Bootstrap、两类代表数据、`initial-migrate`、`app-up`、acceptance 与 resilience，并在数据库快照、TTY、业务 API 或资源启停前阻断；不同 Backup ID 不因此全局互斥。
- `finalize` 的 acceptance/resilience Record lexical 父目录必须分别等于对应 canonical 默认或显式 guarded directory；入口先扫描全候选 acceptance sidecar。resilience final 还必须是 `root:root 0644`、`nlink=1`、稳定且没有 `.tmp-*`/未知 alias/sidecar 的已收口普通文件，candidate 不替 resilience 清理残留。
- Run 34520442209 的其他 8 个 required Job 均通过，但 disposable updater 在 source Backup 后调用 Restore 时暴露了新增必填 `release_record_dir` 未从编排器透传的接线遗漏。当前候选已传入与 Backup 相同的受保护 Release Record 目录，并以专门回归测试同时锁定 Backup/Restore 两端；该失败 Run 不构成 updater PASS，修复仍须由后续当前 SHA 的全新 9/9 证明。
- Run 34523689519 已为 head `41ad3cd...` / merge target `ceb664e...` 完成 9/9；持久 Gate A 随后通过 candidate stage 及全新 M7 Backup/Restore `20260911t013550z`，但旧 Runbook 的宿主 `python3 -m scripts.release.gatea_*` 调用在已安装 Release 生成三个 `__pycache__/*.pyc`，使冻结 source manifest 从 968 变为 971 文件，因而在零写入 `activate-config` plan 被正确阻断。在该历史检查点，现场配置、数据库和四服务仍为 M7，没有 activation journal 或 M8/M9 迁移；该 target 只作诊断证据，不得删除 cache 后续跑。所有已安装 Release CLI 必须显式使用 `python3 -B -m`，校验过的单文件 stage launcher 必须使用 `python3 -I -B <launcher>`；全树完整性校验不忽略 bytecode，并由真实 staged release + fresh subprocess 回归锁定。其后 live 已由 A 进入 M9，当前事实以 A/M9、B/prepared 和 C/pre-install 失败检查点为准。
- candidate、Backup/Restore、Upgrade、acceptance 与 resilience 的不可变 Record/artifact 统一采用同目录随机临时文件、完整写入与 file `fsync`、hard-link no-clobber 和目录 `fsync`，并以 durable journal/证据 digest 支持受控崩溃恢复。acceptance 使用 schema v3 pending 与独立 schema v1 success，Payment ID/No 哈希/成功时间跨两者绑定；resilience final 已可见时只做前后两次现场一致性复验并清理最多一个同 inode writer temp，孤儿/额外 alias 阻断且不会重跑故障演练。所有持久变更入口在 CLI work 前统一安装 SIGHUP/SIGTERM/SIGINT guard，SIGINT 保持 Python `KeyboardInterrupt`；进入必须恢复的窗口后，强制恢复/清理子命令以新 session/独立进程组运行。常驻服务 stop 必须用 Compose `ps` 状态复核；Restore project 最多执行两轮 `down`→inventory，并确认容器、两个临时卷和 internal network 全部消失；candidate 临时镜像也最多执行两轮删除→精确 reference inventory。异常传播固定为 recovery/cleanup failure 优先于原始 work/control error，再优先于恢复阶段的 deferred signal。SIGKILL/断电后不能猜测成功。M9 resilience 必须显式绑定 acceptance Record 路径与确认 SHA-256，前后复验 candidate/Image、Operations/CI、upgrade/replay、代表数据/凭据、五服务、attempt digest 与时间顺序；M9 resilience Record 使用 schema v2，M2/M7/M8 legacy Record 仍为 schema v1。上述仓库能力本身不表示 D 接管、D acceptance/resilience 或 finalization 已执行；A/M9 的 live 事实与 S/M7 的 finalized lineage 必须分开陈述。
- 2026-09-11 的 live Gate A 已由旧候选 A `d6c09482ee0f5583d79bd847e995746c9c6ee1a3` 把配置、数据库和五项常驻服务推进到 M9，但最终 `current` 仍指向已确认的 M7 lineage S `73dca350505d43775fb1ff1158ccf6aabc221998`。A 的 admin-assisted acceptance 在 `order_created` 后读取 T01 bootstrap identity 时使用了绕过 Entrypoint Secret 加载的 Compose `exec` 进程，因而安全失败；自动补偿已取消订单、使两个 fixture 精确 `OFFLINE` 且未删除（不可售）、恢复 Kit 库存并撤销登录会话，没有 Payment/Settlement/Refund、TableSession/Timer/Occupancy，canonical schema v3 failure pending 必须保留，不能手工删除或让 A 原地重跑。
- B `ad2ac8c1eb6633daebf3d66d9b3669e441c3c6dd` 已完成 stage，但其失败验收退休 journal 只到 `prepared`，尚未进入停写或 live verification；该 pending 是新的受保护阻断器，不能手工删除、修改或冒充成功。候选 C 的 schema v3 stage 在 pre-install takeover predecessor validation 中误调用只适用于已安装 Release 的 `_runtime_modules()`，隔离 launcher 因此报出 `activation must run from an installed candidate release` 并在任何 C pending/Image/Release 写入前安全停止。清理后 S/A/B 身份与 A failure、B stage、B prepared 三个 digest 不变，C 无现场残留，不能重用 C。候选 D 的修复在 source/archive/launcher digest 与完整 GitHub provenance 验证后先执行第二次全局 blocker scan，再从归档内以 stable no-follow exact-bytes 方式加载标准库限定的 operations validator；禁止通过 `PYTHONPATH`、`current`、A/B Release 或现场工作树 fallback，takeover validation 未通过前不得写 pending 或 build Image。D 当前本地门槛为 candidate 专项 `207 passed`、Release 完整 `1013 passed`、后端完整 `3135 passed, 39 skipped`；尚未取得自身远端 CI 或执行现场 stage。
- 全新候选 D 的 stage schema v3 必须同时绑定 A failure digest、B stage Record digest 与 B prepared pending digest；D 的 takeover retirement 再按 `prepared` → `write-free-verified` → `superseded-retirement-archived` → `acceptance-archived` → `record-published` → `superseded-retirement-removed` → `canonical-removed` → `runtime-restored` 八阶段收口。B 原 journal 与 A 原 acceptance pending 分别进入 `/srv/pinkdoohub/gatea/records/m9-retirement-failures` 和 `/srv/pinkdoohub/gatea/records/m9-acceptance-failures`；两个目录都必须是 guarded `root:root 0700`，归档文件保持 `root:root 0600`、`nlink=1`、稳定 digest，并作为后续 activation/rollback/adoption/finalize 的持续 lineage 依赖。
- takeover verifier 使用全新 D 镜像，只通过有界 stdin 接收冻结 ID、A acceptance attempt ID 与候选身份，真实进程 argv 必须为空且 schema version 必须是精确整数；它要求同一 Kit 的全部库存流水恰为 SUPER_ADMIN supply `0→10`、订单扣减 `10→9` 和取消恢复 `9→10` 三条，拒绝缺失、额外或字段漂移，并确认该 Order 没有 Payment/Settlement/Refund/WalletTransaction/TableSession。takeover 恢复 allowance 必须把 D 当前 pending、B superseded pending 与 A acceptance sidecar 交叉绑定到同一 A runtime 和精确 checkpoint；普通 work/control error 优先于恢复阶段延迟信号传播。
...
当前仍为 **No-Go**：最后已 finalized 的 lineage 是 M7 Runtime S `73dca350505d43775fb1ff1158ccf6aabc221998`，live config、数据库和五服务是旧候选 A/M9 `d6c09482ee0f5583d79bd847e995746c9c6ee1a3`，B `ad2ac8c1eb6633daebf3d66d9b3669e441c3c6dd` 只留下 `prepared` retirement pending；C 虽有 Run 34616037853 的 9/9，但真实 stage 在零现场写入前失败且已清理。唯一允许的后续是全新候选 D 先取得自身干净 SHA 的远端 required Jobs 9/9，再执行 schema v3 `stage(D, A failure digest, B stage/pending digests)` → 八阶段 takeover retirement → 全新 A/M9 source Backup/Restore → `activate-config` A→D → M9→M9 零迁移 adoption/replay → `app-up` → D acceptance → 显式绑定 acceptance 的 resilience → D 数据后 Backup/Restore → 以 source=S、predecessor=A、target=D `finalize`。禁止手工删除任一 pending、复用 B/C 或旧 Run 的 CI、临时注入 Secret、重跑 M7→M9/M8/M9 迁移、降级数据库或拼接旧证据。Gate A SSH 目标保持 `ubuntu@118.195.195.59`。真实 Origin、`release_eligible=true` RC、iOS/Android 真机、微信官方小程序码、upload/gray/release 与真实微信支付仍未完成或未授权；共享、预发布和生产环境未触碰。
