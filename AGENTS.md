...
- MySQL 8+ 权威 M0–M7 迁移链均已离线生成并通过静态契约测试；本地提交 `58d8435` 对应候选已在一次性 MySQL 8.0.46 真实执行完整 Aerich 0→7 与各历史起点重放，完成 M6/M7 结构快照及 Inventory+Reservation `21 passed`。实例已销毁，未使用 `--fake`，也未应用任何持久、共享或生产数据库；该提交尚未 push，因此不是远端 8/8 证据。
...
当前仍为 **No-Go**：Gate A 持久环境在 2026-09-02 的最后留证为非空 M2，当前真实状态尚未重新只读确认；现有 `initial-migrate` 只支持空库，尚无经批准的 M2→M7 非空升级入口。MARD 色板仍缺持久 MySQL/对象存储导入流程；钱包专项扩展 MySQL 门槛、backfill 与 reconcile 尚未对目标环境完成；候选尚未 push 并获得当前 SHA 远端 8/8；真实 Origin/RC、iOS/Android 真机及微信外部条件未验收。其他已知限制包括真实微信支付 Provider/商户进件未接入、生产色板未经实体拼豆样本校色、Reservation N2 主动通知未实现、邮件验证与 OAuth、管理员启用用户及头像上传仍待实现。M4–M7 均未应用 Gate A/共享/预发布/生产 MySQL；Refresh family 轮换与认证限流已由 Phase 9.5 实现。不要在无关任务中顺手扩展这些范围。
