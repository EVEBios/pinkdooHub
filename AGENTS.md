...
2026-09-09 已新增可复用的本机容量工具，并在 MySQL 8/M8、共享双 CPU、五个稳态容器内存上限合计 4096MiB、唯一 5Mbps TBF 出口下完整执行 A/B/C/D、5/10 VU 的 12/12 Profile 探索轮。`73,027` 个请求均成功，gzip 色板、认证浏览、C v3 的 221 PNG 冷/热完整页面和 150 个订单/库存/钱包写旅程均通过，最终对账、日志/statement 与资源清理通过；唯一失败是 10 VU 持续请求 51,063-byte 未压缩色板，产生 428 个 qdisc drops、P95/P99 1,510/2,442ms，所以整轮严格为 `FAIL`。gzip 后为 10,023 bytes、减少 80.371%，同一 10 VU 为 271/290ms 且零 drops。该 dirty-tree、单轮 ARM64 Docker **服务容器包络**不包含宿主内核/daemon/Runner，不是独立 2C/4GiB 主机、candidate-pre、TLS/公网或真机证据；报告见 `docs/09_release/reports/m8_local_2c4g_5mbps_load_test_2026-09-09.md`。当前本地回归为后端 `2317 passed, 33 skipped`、Release `229 passed`、Performance `190 passed`，前端仍为 `84 suites / 573 tests`；无新增依赖或版本升级。

当前仍为 **No-Go**：持久 Gate A 的权威成功点是 2026-09-08 的 M7 Runtime `73dca350...` 与数据后 Backup `20260908t021224z`；M8 基线 head `4e745848...` / Run 34242753255 已远端 8/8，但尚未应用 Gate A。仓库候选现已新增显式 `gatea_upgrade --source-version 7`，只执行 M8，以版本化 `m7-preserved-business-v1` 对 20 个非 `bead_colors` 表和该表 M7 投影（共 21 个业务表）做内容摘要，并要求停写后的 raw M7 色卡/schema/221 PNG 预检及 Online 目录的事务锁定精确 no-op；旧调用仍默认 M2 并对当前 M7 fail closed。成功升级后必须在 `app-up` 前保持停写并重放 upgrade plan，实时核验数据库、图片、内容摘要和 MARD preview；`app-up` 本身不执行这些 live 检查。独立只读代码审查曾发现成功重放没有重新证明 App/Nginx 仍停服；修复并补齐服务状态 fail-closed 矩阵后复核无未解决 P0–P3，但该结论只绑定本地 diff。当前仍未形成最终干净 SHA，也未完成该 SHA 的远端 CI 和完整 updater 隔离 MySQL 门槛，更没有持久写入授权。仍须绑定新 Backup/独立 Restore、目标 SHA/Image、停写窗口及当次明确授权。保护依赖维护窗口内不存在直接 SQL 或宿主图片旁路写入，不构成跨数据库/文件系统的绝对原子事务。M8 HEX API、小程序直绘、gzip Runtime 和 M8 数据后恢复均未现场验收；真实 Origin/RC、iOS/Android 真机及微信外部条件也未完成。其他限制包括真实微信支付 Provider/商户进件、实体拼豆校色、Reservation N2 主动通知、邮件验证/OAuth、管理员启用用户和头像上传。不得把旧 CI、本地 SQLite 或 M7 历史证据误报成新入口或 M8 持久环境证据，也不要在无关任务中扩展这些范围。
