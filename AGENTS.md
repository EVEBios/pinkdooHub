...
- Product 的业务、数据库、API 和 Schema 契约已完成；`app/common/` 中的 Product Enum/常量、`app/schemas/product*.py`、四个 Product Model，以及 `app/repositories/product_repo.py` 已实现并有契约测试。Product Validator 阶段已完成。Product Service 的首个切片 `online_product()` 已实现：区分不存在/删除/已上线，调用 Validator，并在同一事务中更新状态和写 `ONLINE_PRODUCT` 审计。其余 Product Service 用例和 API 仍待实现，当前端点不可调用。
