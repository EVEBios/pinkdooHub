...
当前仍为 **No-Go**：Gate A 持久环境在 2026-09-02 的最后留证为非空 M2，当前真实状态尚未重新只读确认；现有 `initial-migrate` 只支持空库，尚无经批准的 M2→M7 非空升级入口。MARD 色板仍缺持久 MySQL/对象存储导入流程；钱包专项扩展 MySQL 门槛、backfill 与 reconcile 尚未对目标环境完成；当前 head 远端 8/8 已通过，但真实 Origin/RC、iOS/Android 真机及微信外部条件未验收。其他已知限制包括真实微信支付 Provider/商户进件未接入、生产色板未经实体拼豆样本校色、Reservation N2 主动通知未实现、邮件验证与 OAuth、管理员启用用户及头像上传仍待实现。M4–M7 均未应用 Gate A/共享/预发布/生产 MySQL；Refresh family 轮换与认证限流已由 Phase 9.5 实现。不要在无关任务中顺手扩展这些范围。
