"""Reservation 隔离 MySQL 发布门槛的连接与数据清理。"""
...
async def setup_db() -> None:
    """覆盖共享 SQLite fixture，连接已经完成迁移的隔离 MySQL。"""

    # Tortoise 1.1.7 的全局 Executor cache key 不包含数据库后端；同一
    # pytest 进程跨 SQLite/MySQL 运行前后必须清空，避免复用错误
    # 占位符。
