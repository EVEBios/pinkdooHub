...
class TestRegister:
    ...
    async def test_duplicate_username(self, client: AsyncClient):
        await client.post(
            "/api/v1/auth/register",
            json={"username": "dup", "password": "12345678", "nickname": "First"},
        )
        resp = await client.post(
            "/api/v1/auth/register",
            json={"username": "dup", "password": "87654321", "nickname": "Second"},
        )
    ...
    async def test_short_password(self, client: AsyncClient):
        resp = await client.post(
            "/api/v1/auth/register",
            json={"username": "bob", "password": "123", "nickname": "Bob"},
        )
    ...
    async def test_password_is_hashed(self, client: AsyncClient):
        ...
        await client.post(
            "/api/v1/auth/register",
            json={"username": "hash", "password": "12345678", "nickname": "Hash"},
        )
...
class TestLogin:
    ...
    async def test_login_success(self, client: AsyncClient):
        await client.post(
            "/api/v1/auth/register",
            json={"username": "alice", "password": "12345678", "nickname": "Alice"},
        )
        resp = await client.post(
            "/api/v1/auth/login",
    ...
    async def test_disabled_user(self, client: AsyncClient):
        ...
        await client.post(
            "/api/v1/auth/register",
            json={"username": "bad", "password": "12345678", "nickname": "Bad"},
        )
