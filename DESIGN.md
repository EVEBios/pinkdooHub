...
pinkdooHub 把移动拼豆店组织成一本轻盈但可核对的丝带账簿：暖纸白承载真实商品与运营事实，克制莓果色只标记品牌、当前选择、权威结果和明确下一步。系统已经落到顾客与 ADMIN+ 的 25 条已注册页面路由；登录、注册等认证入口沿用同一视觉语法，业务状态、金额、库存和操作后果始终先于装饰。
...
- 390px 手机视口覆盖 25 条已注册页面路由，会员、钱包与关键工作流同时在 768px 宽屏复核。
...
顾客商品区默认是两列网格；768px 起扩为三列。订单、商品、用户与资金记录在手机上保持单列，关键列表和详情在 768px 使用两列或更宽的动作布局；详情主动作在宽屏通常收束到 420px，而不是横跨整个画布。完成态验证覆盖 25 条已注册页面路由的 390px 手机视口，并对首页、会员、钱包、商品、购物车、订单和核心 ADMIN+ 页面执行 768px 宽屏复核。
