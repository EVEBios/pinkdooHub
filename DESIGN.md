...
系统以紧凑中文层级、短行标题和可扫读数字连接选购与运营。邻近莓色渐变像窄丝带一样组织关键动作，透明与模糊只用于导航、筛选和操作层级，普通内容继续是可读、可核对的实色纸面。顾客一级导航固定为“商城 / 预约 / 购物车 / 会员中心”，商城页头只保留 `pinkdooHub` 品牌、商品任务与必要的购物流程动作；普通 USER 的账户信息和退出归入会员中心，ADMIN+ 则直接进入无顾客底栏的独立店铺工作台。真实商品图片仍是商城的主要视觉内容。
...
- **内容居中（跨端必检）：** 普通文字操作按钮（含页头小按钮、取消、确认及关联跳转）必须优先复用 `src/styles/_patterns.scss` 的 `button-base` 或其主／次按钮模式。按钮容器显式使用 `display: flex; align-items: center; justify-content: center`，重置 `line-height: 1.2`，配合 `box-sizing: border-box`、对称 padding 和最小触控高度。仅设 `min-height`／`text-align: center` 不会垂直居中：2026-09-15 用豆库存页头按钮漏用基础模式，原生行高随字号变化导致文字偏上。不得用空格、换行、单侧 padding、位移文字或把行高写死为按钮高度来补偿；长文案、字号改变、禁用及加载态仍需整体居中。含多行说明的整行导航可以按明确设计左对齐，不得为修一个按钮而全局覆盖所有 Button 布局。
- **居中验证：** 新增／修改按钮后，同时检查源码与实际生成的 WXSS；至少核对正常／较高按钮、默认／较大字号及同一按钮切换文案的表现。H5 构建或 WXSS 编译通过只证明可编译，不能替代微信实际显示检查；发现偏移时先查原生行高、padding、内部包裹元素及样式覆盖顺序。
...
四个顾客一级入口固定为“商城 / 预约 / 购物车 / 会员中心”，顺序和根路径不因身份变化。微信端使用悬浮“瓷白丝带托盘”自定义 TabBar：托盘左右留出呼吸空间，包含底部安全区，四项严格等宽且每项保持源 88px／H5 44px 的最小触控区域。未选中项使用柔莓墨灰轮廓图标与常规字重；选中项使用深莓圆形底座、白色实心图标、加粗莓果文字和克制上浮，同时用形状、填充、字重和位置表达状态，不只依赖颜色。图标使用同一授权图标库导出的本地 PNG 状态族，保留来源与许可证，不使用 emoji、手绘 SVG 或其他品牌资产。
...
商城、预约、购物车和会员中心四个根页面显示底栏；无底栏启动分流页、店铺工作台、商品详情、订单列表、下单确认、订单/预约详情、登录注册、钱包充值/流水及所有管理页不显示。进入其他顾客根页统一使用 `switchTab`，重复选择当前页不得堆叠页面。Guest 在受保护根页看到登录引导，普通 USER 才展示自己的预约、订单和钱包；ADMIN+ 在服务端身份确认后使用 `reLaunch` 进入工作台，误入任一顾客根页时也回到工作台，且不挂载顾客商品、预约、订单或钱包请求。支付宝、抖音和 H5 使用同一四项顾客信息架构及本地图标的原生 TabBar 兼容降级，不要求复制微信悬浮造型。

会员中心仅服务 Guest 和普通 USER：USER 使用连续莓红纹理身份／订单区，下面依次为横向余额卡、完整展开的个人资料和浅粉胶囊退出按钮；ADMIN+ 不在这里维护第二份管理入口。页头只保留 `pinkdooHub` 品牌、页面任务与必要业务动作，已删除的右上角“拼豆店”标签不得恢复。
...
管理订单详情的辅助区域采用轻量点缀：下单用户和商品卡使用柔粉内嵌细边线，小计独立淡粉底，时间／备注区使用浅粉实色；资金标题使用浅粉标题带和随圆角裁切的窄莓红顶线。装饰仅表达分区，不改变订单、收款、退款、禁用状态的语义。
...
### Store Bead Stock Ledger（局部表面）

门店用豆库存采用用户选定的 Product Design「柔粉分区」（本轮 Option 3）：瓷白基底、浅粉筛选分区、双列紧凑台账和按可见行交替的浅粉底色，莓果仅用于选中态、变更文字及主要操作。0/1 包不带红边或告警，隔行底色与数量无关。两组筛选完整显示，不横向滚动。常规手机宽度使用横向减号／数字／加号；360px 及以下将数字独立成行，保留双列与最小 44px 触控范围。编辑原因和备注使用紧凑输入行。

二次确认按用户补充保留独立记录行：左侧色号和增加／减少包数，右侧修改前 → 修改后；首次盘点显示未盘点 → N 包。顶部单独汇总增加与减少，原因／备注使用浅粉分区，底部两个按钮保留明确间距。历史详情复用相同变更展示。此次仅调整本页面局部样式与结构，未更改全局设计令牌和业务规则；[本轮视觉验证](docs/08_frontend/qa/store-bead-stock-soft/review.md)记录选图来源、实屏和证据范围。此前功能已获用户确认，新版微信真机视觉仍待用户查看。

### Customer Textured Surfaces

商城、预约、购物车和会员中心四个根页，以及商品详情、订单列表、下单确认、订单／预约详情和钱包流程，复用同一真实深莓分块纹理作为顶部身份区；白色标题与近白粉副标题建立稳定对比，各页只调整 `background-position`，不另造互相竞争的主题。真实商品图片仍保留在商品内容区，纹理不进入商品图片、业务状态、错误、表单正文或高密度流水。

顾客侧瓷白筛选、摘要、表单与记录卡使用窄内嵌强调带建立轻量分块节奏，色带由容器圆角裁切，不在角落拼接两条边框。强调带只服务层级，不替代成功、警告、危险、禁用等既有文字与语义色。

### Member Balance Ribbon

会员余额采用瓷白横向卡：左侧是可用余额标题、资金明细与充值状态说明，右侧是带露出纸钞和搭扣的柔莓玫瑰色钱包图案及深莓红金额，以细竖线分隔。资金明细标题区保留完整触控高度；未开通充值使用紧凑的静态说明，不以空高按钮撑开三行内容。钱包置于正方形容器并以 aspectFit 等比显示，和金额一组居中；长金额适当缩小字号并保持单行，不截断。充值、支付与退款仍由服务端能力和账户状态驱动；充值能力开启且账户可用时才展示可点击充值入口；未开通状态不触发导航或支付。

个人资料位于余额之后，不再折叠或重复放头像；昵称／账号与手机号／状态／加入日期分组展示，日期只展示服务端 UTC 日期部分，原始资料不变。退出按钮采用浅粉实底、莓红文字、无描边胶囊，仅在真实退出处理中显示禁用状态。余额上限与用途说明保留在页面末尾，滚动尾部避让底栏。
...
次级详情以浅粉标题带（#faedf2）、柔莓细侧线（#c97991）组织信息；时间、备注、联系人等次级摘要可用柔粉底（#fcf3f6）。只按需应用，保持基础白卡样式。管理筛选区的窄内嵌顶线不得被后续 box-shadow 覆盖。登录／注册只补窄顶线；桌台使用既有纹理页头与浅粉卡片标题，待支付／体验中继续使用语义色，长编号允许换行。已有完整视觉层次的商城、会员、工作台等不叠加装饰。
...
## 2026-09-14：预约提醒 P1.1

保留 Ribbon Ledger，预约入口增加紧凑红底白字数字角标，触控目标仍为整行；0 隐藏，99+ 扩宽，不闪烁。管理员第一组改为“待处理”，副文案拆出待审核与取消变更，避免未来预约被误解为仅今天。顾客首页/预约页使用同一紧凑结果入口，预约 Tab 同步数量，提醒列表作为二级页。业务状态和提醒阅读状态分别表达，详情错误保留重试。

实现与页面证据见 [P1.1 页面检查](docs/08_frontend/qa/attention-review.md)，业务规则见 [提醒需求](docs/01_requirements/attention_module.md)。订单、桌台与资金角标后续已按 P1.2/P1.3 接入，沿用同一提醒样式。

### 预约待办与过期修订（2026-09-14）

待办页复用既有管理纹理页头和圆角白面板，筛选等宽胶囊、行内数字，正文使用既有预约记录卡。操作指引随当前筛选变化，空态独立成组并提供过期记录入口。“已过期”以浅色状态显示，区别可审核等待色。证据：[设计检查](docs/08_frontend/qa/attention-refinement-review.md)。

### 会员提醒入口（2026-09-14，Option 2）

会员中心订单／桌台提醒通过 `CustomerAttention appearance="member"` 融入深莓身份区：浅粉透明底、细描边、圆角图标座，白色加粗标题与近白粉说明上下排列，右侧独立数字角标和白色箭头。整行是唯一触控入口，说明允许自然换行，不使用横向滑轨。会员“我的桌台”固定展示，有提醒时说明为“查看待付款与桌台动态”，无提醒时为“查看进行中与历史桌台”；数量继续来自服务端，0 仅隐藏角标、超过 99 显示 99+。首页保留原浅色提醒样式。钱包加载失败时，提醒与订单快捷入口由同一深莓容器承载，保证白字可读。

页面证据与边界见 [会员提醒设计检查](docs/08_frontend/qa/member-attention-review.md)。


### 资金结果 P1.3（2026-09-14）

余额变动入口复用会员现有深莓底、图标座、两行文案和数字角标。余额列表只提供“全部调整／新结果”两个等宽筛选，不采用横向滚动。详情沿用提醒页页头和结果面板，按金额变化、调整前后余额、完整原因与时间展示；按钮使用既有次要按钮样式及显式间距。已读保留正文，余额标注“本笔发生时”，避免与会员当前可用余额混淆。

## 2026-09-15 · 预约跟进

工作台追加“预约跟进”入口，继承 Ribbon Ledger。列表用两行双列筛选；详情表单双列结果、必填说明与显式按钮间距。成功、明确失败、未知结果分别用既有 success/danger/warning。H5 内部输入框使用类选择器，避免 Taro 标签转换；可用按钮省略 disabled 属性。320/390 截图及独立设计复核通过，仅证明本轮 H5 UI；见 [检查记录](docs/08_frontend/qa/p2-followups-review.md)。
